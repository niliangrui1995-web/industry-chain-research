...
- 涉及一个或多个外部/本地市场数据字段时，先自动按标的、字段、`as_of`、历史区间、口径和证据等级拆成数据卡，再读取 `docs/data_source_routing.md` 选择每张卡的主数据源和有序等价回退；用户无需手动指定供应商。不得无差别调用所有数据源；主源必须先通过最小只读探测，回退也必须字段、日期和口径等价。用户明确指定来源时优先遵从，领域 Skill、自动化合同和受控产物的硬性来源规则优先于通用路由。
- 账户级可用的数据能力（包括 `hithink-finance`）仅按 `docs/data_source_routing.md` 的字段匹配、口径和当次可用性按需选用，不在本文件设置固定默认优先级。
