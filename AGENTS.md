...
- Use `skills/ht-local-market-data` only as a read-only local `C:\zd_huatai` HT/TongdaXin market-data layer for post-close daily K-lines, block pools, vendor tables, and freshness checks; it is not official evidence and must not inspect trading-account/password/order/log details unless explicitly requested.
