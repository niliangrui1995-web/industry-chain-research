...
模型价格和交易价格严格分栏。模型价只用截至样本 cutoff 已生效的公司行动顺时间累计；真实回测仍使用原始成交价。窗口在 cutoff 处缩放回原始收盘量级。
...
公开数据原始层由 `scripts\kronos_a_share_public_data.py` 提供三个受控函数：`snapshot_url_manifest()` 保存显式 HTTPS 官方/公开响应、最终 URL、抓取时点、工件身份和 SHA256；`fetch_baostock_trade_status_shards()` 分证券保存 `tradestatus/isST` shard 及 manifest；`publish_normalized_pit_bundle()` 按 `kronos-a-share-pit-normalization-v1` 显式列映射把已抓原始响应发布为七张业务表、`coverage`、原始副本和逐表 provenance。归一化优先级固定为 `official_primary > public_secondary > tdx_mechanical`；TDX 只能是 `mechanical_cross_check`，不得供值。同优先级异值键与 TDX 核验冲突键会被排除；只有配置另一个受原始快照哈希绑定的 `expected_keys`、无缺键/冲突且来源有效期无缺口时，对应 coverage 才可为 complete。未提供 `expected_keys` 不会推测完整性，只能发布为 `local_provisional`。

归一化配置示例为 `configs\pit_normalization_v1.example.json`。先将占位路径和列名替换为已审核原始快照的真实合同，再在不新增第十个命令的前提下运行：

```powershell
$AsharePython = 'D:\vcp_hunter\产业链投研\_training\kronos_ashare\runtime\venvs\kronos-ashare\Scripts\python.exe'
& $AsharePython .agents\skills\kronos-market-forecasting\scripts\run_kronos_a_share.py prepare --config .agents\skills\kronos-market-forecasting\configs\a_share_daily_v1.yaml --pit-root 'D:\vcp_hunter\产业链投研\_training\kronos_ashare\data\normalized\pit\pit-v1' --pit-normalization-manifest .agents\skills\kronos-market-forecasting\configs\pit_normalization_v1.reviewed.json
```
...
正式训练额外执行反向幸存者审计：每个点时 CSI300/CSI500 成分在每个交易日都必须有 `.day` 记录，缺记录只能由同日 PIT `suspensions.is_suspended=true` 解释；缺整只证券、静态现存股票池替代历史成分或事后补齐都会阻断 `production_ready`。

## 两阶段训练

第一阶段只训练26个 LoRA 目标：12层 self-attention 的 `q_proj/v_proj`，以及 dependency cross-attention 的 `q_proj/v_proj`。固定 `r=8, alpha=16, dropout=0.05`，应有346,112个参数。损失只覆盖 token 序列中未来10根位置。
...
默认 Windows 单进程、FP32、`num_workers=0`。Adapter 为 `batch=16`、累积2；1000步冒烟后从相同配置哈希续训到最多10000步。验证固定为 `causal-dependency-cross-attention-v1`：dependency cross-attention 在验证期继续使用因果 mask，而 LoRA dropout 关闭。Checkpoint 使用临时目录、完整文件哈希、`COMMITTED` 和原子 pointer；配置、基础模型、Tokenizer或数据 manifest 漂移时必须新建 run。

## 评估和准出

至少比较 last-value、漂移、动量/反转、zero-shot Kronos、head-only 与项目内 Qlib Alpha158+LightGBM。不得直接采用 Qlib 示例中的固定 `limit_threshold=0.095`，A股交易规则必须逐日版本化。

正式 `validation` 在当前 token cache 上现场重算 zero-shot/adapted CE 和 scorer 预测，逐张量核对 scorer 与声明 adapter 的 canonical LoRA digest，并从绑定数据重建所有基线和成交列；checkpoint extra、旧 predictions、companion 及其可改写 metadata 只作审计，不能代替重算。`development_test` 与 `locked_retrospective` 只生成各自受控报告和分段工件，不创建、替换或回滚 gate/head。
...
失败工件保留用于诊断，但输出固定为 `output_type=N/A`。通过也只能称 `evidence_class=model_output`。
...
& $AsharePython $AshareCli train-adapter --config .agents\skills\kronos-market-forecasting\configs\a_share_daily_v1.yaml --resume auto --stop-after 1000
