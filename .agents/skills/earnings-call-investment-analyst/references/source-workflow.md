...
- Treat earnings calls, earnings webcasts, results briefings, investor meetings, corporate briefings, management Q&A, official captions, replay audio, and replay video as eligible call/event content. For Japan, Taiwan, Korea, Hong Kong, and Europe, label the event type precisely instead of assuming a U.S.-style quarterly conference call.
- Before searching prior-quarter sources, resolve the prior-quarter identity from the task's current fiscal period and official company materials. For month-ended periods, subtract three months, for example `Mar/2026 -> Dec/2025`. For fiscal-quarter labels such as `FY2026 Q3`, verify the company's fiscal calendar from official filings or IR materials. If the current period remains unresolved, label `prior-quarter period unresolved` and avoid unsupported QoQ or prior-call comparisons.
...
- If the webcast exposes an HLS caption playlist, optionally run `scripts/caption_playlist_fetcher.py --playlist-url <subtitles.m3u8>`. Establish the event's official provenance before treating its captions as official; captions do not replace audio-first transcription. Require exit code 0 and manifest `playlist_complete=true`: `playlist_ended` only means an `EXT-X-ENDLIST` tag exists, while `download_complete` requires a nonempty list and successful WebVTT retrieval for every segment. Missing segments, invalid responses, empty lists, or live windows return exit code 1; partial files are diagnostic only. Even a complete download needs an event/date and duration coverage check before it can represent the full call.
- This helper accepts subtitle media playlists with independent WebVTT segment files and finite nonnegative `EXTINF` durations. It rejects master playlists, gap markers, byte ranges, encryption keys, initialization maps, and partial-segment formats until a compatible downloader is used; it must never silently treat an unsupported representation as complete. `playlist_valid` records this format check.
