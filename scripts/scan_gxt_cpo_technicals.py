# -*- coding: utf-8 -*-
"""
扫描通达信概念板块「光通信 880670」与「CPO概念 880656」全部个股的纯技术面快照。
板块成分来源: D:\\HT\\T0002\\hq_cache\\infoharbor_block.dat (前缀 0=SZ 1=SH 2=BJ)
行情来源: D:\\HT\\vipdoc 本地日线(不复权) -> 除权缺口识别 + 前复权近似。
输出: deliverables/technical-analyst/2026-09-23/gxt_cpo_technical_snapshot.csv + 板块成分清单 CSV。
...
BLOCKS = [('光通信', '880670'), ('CPO概念', '880656')]

# ---------- 板块成分解析 ----------
def parse_blocks():
    data = open(os.path.join(CACHE, 'infoharbor_block.dat'), 'rb').read().decode('gbk', errors='ignore')
    out = {}
    # 每个板块: 头部行 #GN_名称,数量,880xxx,... 后续行为 市场#代码 列表, 以下一个 #GN_ 或文件结束为界
    parts = re.split(r'\r\n(?=#GN_)', data)
    for p in parts:
        m = re.match(r'#GN_([^\r\n,]+),(\d+),(88\d{4}),', p)
        if not m:
            continue
        name, cnt, code = m.group(1), int(m.group(2)), m.group(3)
        members = re.findall(r'(\d)#(\d{6})', p)
        out[code] = (name, cnt, members)
    return out

block_data = parse_blocks()
...
results = []
rosters = {}
for bname, bcode in BLOCKS:
    if bcode not in block_data:
        print(f'!! 板块 {bname}({bcode}) 未在 infoharbor_block.dat 中找到')
        continue
    nm, cnt, members = block_data[bcode]
    seen, roster = set(), []
    ...
    print(f'{bname}({bcode}): 头部声明 {cnt} 只, 实际解析 {len(members)} 条, 去重后 {len(roster)} 只; 更新日期见头部')
...
cols = ['block', 'mkt', 'code', 'name', 'last_date', 'close', 'pct_chg', 'chg5', 'chg10', 'chg20',
        ...
        'dif', 'dea', 'macd_bar', 'macd_cross5', 'dif_above0', 'rsi6', 'rsi14',
        'kdj_k', 'kdj_d', 'kdj_j', 'boll_pos', 'vol_ratio', 'vol5_vs20', 'turnover',
        'hi20', 'lo20', 'hi60', 'lo60', 'from_hi60', 'range_pos60', 'limitups_20',
        'pv_diverge', 'stale', 'ex_gaps_recent', 'error']
csv_path = os.path.join(OUT_DIR, 'gxt_cpo_technical_snapshot.csv')
...
if len(rosters) == 2:
    ...
    print(f'\n两板块交集 {len(s1 & s2)} 只; 仅光通信 {len(s1-s2)}; 仅CPO {len(s2-s1)}')
