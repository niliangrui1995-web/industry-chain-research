...
- **未改变（链条边界）**：不改变 `2026-08-09` 主台账的 `as_of`、check_id 或 hard/soft 等级。公司披露的 backlog/新净承诺仅增加需求可见度，收入仍取决于容量上线与服务可用性，不能写成客户订单、收入覆盖、已锁定指引或现金流承诺。公司定义 CapEx 也不能与现金流量表购买 PPE/资本化软件混算。
- **电力/液冷边界与仍待验证**：电力、冷却、硬件、存储、网络只是管理层列示的多年期投入保障与风险缓释项；数据中心电力/液冷设施集成维持低置信度 `watch`，不升级为行业供给缺口、供应商订单或已解除瓶颈。待 Q2 10-Q、active power/已上线项目、backlog 转化和覆盖率、取消/推迟，以及单品 lead time/项目延期披露验证。
- **审计与来源**：以上三个环比数字分别对应 `C_QOQ_REVENUE`、`C_QOQ_ADJ_EBITDA`、`C_QOQ_CAPEX`，审计结果为 `publishable`；依据：[CRWV FY2026 Q2 财报复盘](../../earnings/CRWV/Jun_2026/analysis/earnings_review.md)。
