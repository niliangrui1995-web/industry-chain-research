# 300394.SZ 天孚通信 跟踪状态

baseline_status: done
last_checked: 2026-07-20 20:58 Asia/Shanghai
browser_scope: not_used_by_policy
grok_status: not_used_by_policy
open_web_fallback_status: searched_no_new_nonofficial_signal_2026-07-20
announcement_window_checked: T_and_T_plus_1
not_investment_advice: true

## 当前跟踪结论

天孚通信按“AI 数据中心光器件平台供应商 + 精密无源/高速有源双主线 + CPO 配套 FAU/ELS + 1.6T 光引擎 + 海外产能”跟踪。硬证据来自 2025 年报、2026Q1、2026-05-14 IR、H 股申请公告和新加坡合资公告。

公司质量和业绩弹性都较强，但当前只能按公司级窄口径承接光模块主状态：FAU/ELS 已稳定交付，1.6T 光引擎已规模量产，个别物料仍阶段性缺货；仍缺订单、收入、毛利率、客户和产能锁定量化，不能把 CPO、1.6T、NVIDIA、海外 CSP 或光模块客户扩产叙事直接写成公司确认订单。

## 2026-07-10 日更与官方 IR 延迟补录

- 公告窗口：北京时间 20:00 后完成 `2026-07-10` 与 `2026-07-11` 的 T/T+1 检查；CNINFO 未发现本窗口新增发行人公告。逐公司 open-web 检索命中 7 月 9 日可信媒体对天孚通信调研内容的转述，由此回到 CNINFO 补录 6 月 28 日《投资者关系活动记录表》（调研日期 6 月 27 日）。
- 延迟补录的官方 IR：泰国工厂无源产品线稳定量产并按客户需求扩产，有源产品线已通过客户审核，预计下半年产能利用率提升；CPO 配套 FAU、ELS 外置光源处于稳定交付状态；公司预计未来几年持续增加资本开支。Q1 光引擎以单通道 200G、EML 路线为主，公司预计个别物料供应紧张从下半年开始逐步缓解。
- 来源边界：每日经济新闻 7 月 9 日报道补充了 A/B 栋、200G EML 供给缓解和泰国公司扭亏等更细颗粒信息；未取得对应新增官方原文的部分只保留在可信媒体观察层，不升级为正式披露。
- 龙虎榜 / 大宗交易：2026-07-10 未发现天孚通信新增龙虎榜或大宗交易；2026-07-09 回扫无新增延迟数据。
- 状态变化：泰国有源产线通过客户审核、CPO 配套稳定交付和物料紧张预计缓解，增强产能爬坡与交付能力证据；仍缺泰国产能利用率、收入、毛利率、客户及 CPO/1.6T 订单量化，不能直接上调到完整业绩兑现。

## 2026-06-20 光模块主状态窄口径同步

- FAU/ELS：公司 2026-05-14 IR 已披露面向 CPO 领域配套的 FAU、ELS 外置光源等产品实现稳定交付；按稳定交付/产品验证处理，不升级为当前行业硬堵点。
- 1.6T 光引擎：2025 年报已披露完成规模量产；按已量产处理，但不等同于已披露订单、收入占比、毛利率或客户量化。
- 个别物料：仍维持公司级 `soft_bottleneck/watch`；缺具体物料类别、缺口比例、交期和客户影响，不能反推 FAU/ELS 或行业级短缺。
- 主状态外部观察：FAU/fiber array、主动对准/WLBI、ELSFP thermal 在光模块主状态中仍是未来迁移观察，不写成天孚确认订单、收入或毛利率兑现。
- 量化缺口：“CPO 光引擎市占 >60%、订单排至 2028”等说法未取得官方硬证据；后续只在公司披露订单、收入、毛利率、客户或产能锁定后升级。

## 最新硬事件

- 2026-05-14：投资者关系活动记录披露，公司面向 CPO 领域配套的 FAU、ELS 外置光源等产品已实现稳定交付；北美洲 2025 年收入 6623 万元、占收入 1.28%，公司称关税变化对整体业务直接影响相对有限；个别物料仍存在阶段性缺货。
- 2026-04-22：全资子公司新加坡天孚拟持股 35% 设立 SuperX Optical Communications Pte. Ltd.，合资公司经营范围为光互连产品，出资额 200 万新币。
- 2026-04-21：2026Q1 收入 13.303 亿元，同比 +40.82%；归母净利润 4.922 亿元，同比 +45.79%；扣非净利润 4.932 亿元，同比 +48.44%；经营现金流 1.823 亿元，同比 -37.92%。
- 2026-04-10：公司向香港联交所递交 H 股发行并上市申请，仍待相关监管批准、核准或备案。
- 2026-04-08：2025 年报披露收入 51.634 亿元、归母净利润 20.173 亿元；有源光器件收入 29.979 亿元；公司称完成 1.6T 光引擎规模量产和 CPO 配套光器件研发。

## 2026-07-06 日更

- 公告窗口：北京时间 20:00 后完成 `2026-07-06` 与 `2026-07-07` 的 T/T+1 检查，并补扫 `2026-07-05`；CNINFO 确认 2026-07-07 新增《关于独立董事取得独立董事资格证书的公告》。
- 新增官方治理事项：独立董事杜琳琳女士已参加深交所上市公司独立董事任前培训并取得《上市公司独立董事培训证明》；其任期自公司 H 股发行上市之日起至第五届董事会任期届满之日止。
- 龙虎榜 / 大宗交易：2026-07-06 未发现新增龙虎榜或大宗交易。
- 状态变化：该事项属于低影响治理/合规补齐，不改变 CPO 配套 FAU/ELS、1.6T 光引擎、泰国产能、客户集中或 H 股发行审批主线；后续仍看 H 股监管节点、CPO/1.6T 收入和毛利率量化。


## 2026-06-09 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-09` 与 `2026-06-10` 的 T/T+1 检查；CNINFO、SZSE 口径未发现新增公告、IR 或 H 股进展。
- 龙虎榜：2026-06-09 未发现天孚通信新增龙虎榜。
- 大宗交易：2026-06-09 发生 2 笔平价大宗交易，成交价均为 443.92 元，与当日收盘价一致；合计成交 305,400 股，成交金额 135,594,500 元，买方均为机构专用，卖方分别为国信证券深圳后海分公司和中信证券北京分公司。
- open-web：普通联网搜索主要返回行情、资金流、公司资料和既有 1.6T/CPO/H 股申请线索，未发现新的公司确认订单、客户、收入占比、毛利率或 H 股审批进展。
- 状态变化：新增高位机构席位平价换手信号，交易结构需继续跟踪；基本面主线不变，仍需等待 CPO 配套 FAU/ELS、1.6T 光引擎、泰国产能、物料供应、客户集中度和 H 股审批的硬证据。

## 2026-05-14 手工新增关注与首次研究

已将天孚通信新增为 watchlist enabled 公司并完成首次基线。2026-05-14 当日有投资者关系活动记录，构成本次新增的最新公司 IR 硬事件。

龙虎榜方面，同花顺页面显示最近 1 个月内该股未能登上龙虎榜。大宗交易方面，公开页面显示最近记录为 2026-05-08 两笔大宗交易，成交价均为 326.30 元，成交金额分别为 2447.25 万元和 8048.29 万元；未见 2026-05-14 新增大宗交易记录。

## 2026-06-30 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-30` 与 `2026-07-01` 的 T/T+1 检查；CNINFO 公司名/代码查询确认新增《关于全资子公司对外投资设立合资公司的进展公告》。
- 新增官方事件：新加坡天孚、SUPERX AI SOLUTION LIMITED、APEX VERVE LIMITED 共同设立的 SUPERX OPTICAL COMMUNICATIONS PTE. LTD. 已完成工商注册登记并取得注册证明，注册资本 200 万新币；新加坡天孚已按合资协议实缴出资 70 万新币，持股 35%。
- 龙虎榜：2026-06-30 未发现天孚通信新增龙虎榜。
- 大宗交易：东方财富大宗交易数据显示 1 笔小额平价交易，成交 1.00 万股、成交价 302.68 元、成交金额 302.68 万元，买方华泰证券福建分公司，卖方机构专用；金额较小，仅作交易结构观察。
- 状态变化：SuperX 光互连合资公司从协议推进到注册和实缴落地，海外/AI 光互连业务布局确定性增强；但仍不能写成 CPO、FAU/ELS、1.6T 光引擎的新增订单、客户、收入占比或毛利率兑现。

## 关键假设

1. 有源光器件占比提升和 1.6T 光引擎规模量产后的放量，是 2026 年业绩弹性的核心。
2. CPO 配套 FAU/ELS 已披露稳定交付，但订单、收入占比、客户和毛利率量化仍是叙事能否升级为基本面硬证据的关键。
3. 泰国基地爬坡和新加坡总部/合资公司有助于全球化交付，但短期仍需看良率、产能利用率和客户导入。
4. 客户集中度极高，第一大客户和前五大客户需求变化会显著影响收入波动。
5. 个别物料阶段性短缺仍是公司级 `soft_bottleneck/watch`，在拆清物料类别、缺口比例、交期和客户影响前，不写成 FAU/ELS 或行业硬堵点。

## Source gaps

- 未取得 Grok/X 原生社交线索；本次为手工新增公司基线，使用官方公告、IR 和普通网页交易数据。
- CPO 配套 FAU/ELS 已披露稳定交付，缺收入、订单、客户和毛利率量化。
- 1.6T 光引擎已披露规模量产，但缺产品收入占比、客户导入和盈利能力拆分。
- 个别物料仍为阶段性缺货，缺具体物料类别、缺口比例、交期和客户影响。
- 主动对准/WLBI、ELSFP thermal 仍按光模块主状态的未来迁移观察处理，缺天孚直接订单、收入、毛利率或客户验证。
- 泰国产能爬坡缺产线利用率、良率、单季度贡献和客户认证进度。
- H 股发行上市仍待监管流程推进。

## 后续跟踪问题

1. CPO 配套 FAU/ELS 稳定交付是否在后续 IR 或财报中披露订单、收入占比、客户和毛利率。
2. 1.6T 光引擎从规模量产到收入放量的节奏、客户导入和利润率。
3. 泰国产能是否在 2026 年持续提产并改善海外客户交付。
4. 个别物料短缺是否缓解，是否披露具体物料、缺口比例、交期和客户影响。
5. 主动对准/WLBI、ELSFP thermal 是否出现与天孚直接相关的订单、收入、毛利率或客户验证。
6. H 股申请何时取得关键监管进展，募资用途是否进一步明确。

## 2026-05-19 batch_no=2 补齐复核

- 公告窗口：已在北京时间 20:00 后补扫公告日期 `2026-05-19` 和 `2026-05-20`，未发现新增官方公告、深交所披露或公司 IR 硬事件；completion 记录 `announcement_window_checked=T_and_T_plus_1`。
- 新增交易事件：2026-05-19 出现两笔大宗交易，成交价均为 363 元，合计成交 276,200 股，成交金额 100,262,400 元，买方均为机构专用。
- 投资含义：这是交易结构信号，成交价等于当日收盘价，暂不改变精密无源/高速有源/CPO/1.6T 主线基本面判断。
- open-web fallback：仅发现 CPO 板块回调类市场观察，不写成公司确认订单或基本面事件。

## 2026-05-20 batch_no=2 例行复核

- 公告窗口：北京时间 20:00 后复核公告日期 `2026-05-20` 和 `2026-05-21`，CNINFO/SZSE 口径未发现新增官方公告或公司 IR 硬事件；completion 记录 `announcement_window_checked=T_and_T_plus_1`。
- 龙虎榜：东方财富龙虎榜数据口径未检出 2026-05-20 天孚通信上榜记录。
- 大宗交易：2026-05-20 出现 1 笔大宗交易，成交价 358.99 元，成交 23,800 股，成交金额 8,544,000 元，买卖方均为机构专用，折溢价 0%；金额较小，仅作交易结构观察，不追加为 material event。
- open-web fallback：未发现近 24 小时公司级新增硬事件；网页结果主要是 5 月 20 日大宗交易、行情/资金流和既有 CPO/FAU/ELS 信息重复，不写成公司确认订单或基本面变化。

## 2026-05-22 日更

- 公告窗口：北京时间 20:00 后检查公告日期 `2026-05-22` 和 `2026-05-23`，CNINFO 检出《更换持续督导保荐代表人公告》，2026-05-23 未见新增。
- 新增硬事件：华泰联合证券因原保荐代表人沈树亮工作变动，委派朱辉接替其履行 2020 年定增项目持续督导职责；更换后持续督导保荐代表人为钱亚明、朱辉。
- 交易事件：2026-05-22 未发现新增龙虎榜或大宗交易，最近大宗交易仍为 2026-05-20 小额平价记录。
- 状态变化：合规类人员变更入账，不改变 CPO/1.6T 光引擎、FAU/ELS、泰国产能、客户集中或 H 股发行主线。

## 2026-05-24 batch_no=2 例行复核

- 公告窗口：北京时间 20:00 后检查公告日期 `2026-05-24` 和 `2026-05-25`，CNINFO/SZSE 未发现新增官方公告、深交所披露或公司 IR 硬事件；HKEX 仅见既有 H 股申请材料线索，未见本窗口新增进展。
- 交易事件：最近 A 股交易日按 `2026-05-22` 处理；深交所竞价交易公开信息未检出天孚通信龙虎榜记录，深交所权益类证券大宗交易未检出 `2026-05-22` 新增记录，最近公开大宗交易仍为 `2026-05-20` 小额平价记录。
- Grok/X：Chrome/Grok 单独标签页查询显示过去 24 小时无新增高价值公司级信号；返回内容主要为 CPO/1.6T/FAU/ELS、泰国产能、H 股申请和物料供应的旧信息重复或市场讨论，不写成确认事实。
- open-web fallback：普通网页补充搜索未发现可写入事件账本的新增硬事件；观察到 `2026-05-25` 有约 301.10 万股限售解禁日历项，来源指向 2025 年 11 月询价转让锁定期安排，暂列交易供给观察，不作为新增公司基本面事件。
- 状态变化：核心判断不变；继续跟踪 CPO 配套交付量化、1.6T 光引擎收入/毛利率、泰国产能爬坡、物料供应、H 股发行审批和小比例解禁后的交易供给影响。

## 2026-05-28 盘中 worker 复核

- 公告窗口：北京时间 `2026-05-28 13:40`、20:00 前检查 `2026-05-26` 至 `2026-05-28`，CNINFO 与 SZSE 上市公司公告口径均未检出新增官方公告、交易所披露或投资者关系活动记录；公司 IR/官网与 HKEX 公开线索未见本窗口新增 H 股发行进展，completion 记录 `announcement_window_checked=pending_evening_rescan`。
- 交易事件：深交所竞价交易公开信息按 `300394` 检索 `2026-05-26` 至 `2026-05-27`，未检出龙虎榜记录；深交所权益类证券大宗交易按同一窗口检索，未检出协议交易或盘后定价大宗交易。`2026-05-28` 尚未收盘，最终交易公开信息和大宗交易数据需晚间或次日复扫。
- Grok/X：当前工具环境没有 Chrome/Grok 可调用入口，记录为 `grok_status=unavailable_no_chrome_grok_callable_tool`；已运行 open-web fallback，结果主要为行情、板块和既有 CPO/1.6T/H 股申请叙事重复，全部仅作 `observation_only`，未发现可写入事件账本的公司确认订单、H 股审批或物料供应新硬证据。
- 状态变化：不追加 `events.jsonl`，核心判断不变；继续跟踪 CPO 配套 FAU/ELS 交付量化、1.6T 光引擎收入与毛利率、泰国产能爬坡、关键物料供应、客户集中度和 H 股发行审批节点。

## 2026-05-28 晚间公告窗口复扫

- 公告窗口：北京时间 `2026-05-28 20:32` 后按公告日期 `2026-05-28` 与 `2026-05-29` 复扫，CNINFO 公司精确查询 `300394,9900023911` 未检出新增公告；CNINFO 当日全量深市公告筛选未见天孚通信；SZSE/公司 IR/官网与 HKEX 公开线索未见本窗口新增 H 股发行进展。
- 交易事件：深交所竞价交易公开信息按 `300394` 精确检索 `2026-05-28` 与 `2026-05-29`，未检出龙虎榜记录；深交所权益类证券大宗交易按同一窗口检索，未检出新增大宗交易记录。
- open-web fallback：因无可调用 Chrome/Grok 工具，记录 `grok_status=unavailable_no_callable_chrome_tool`；普通联网搜索仅发现光模块板块与“易中天”股价创新高、既有 1.6T/物料供应/IR 内容重复等市场观察，不作为公司确认订单、H 股审批或基本面硬事件。
- 状态变化：仅将晚间复扫状态从 `pending_evening_rescan` 补齐为 `T_and_T_plus_1`；未追加 `events.jsonl`，核心判断不变。

## 2026-05-30 日更状态
- 检查窗口：公告 T/T+1（2026-05-30 至 2026-05-31）、最近交易日龙虎榜/大宗交易（2026-05-29）。
- 结论：未发现新的公告、龙虎榜、大宗交易或H股发行进展；1.6T/CPO/泰国产能仍等待官方订单、收入或客户验证证据。
- 来源分层：官方披露/交易数据优先；Grok/X不可调用时使用 open-web fallback，相关线索仅作观察池。

## 2026-06-05 batch_no=2 晚间复核

- 公告窗口：北京时间 `2026-06-05 20:37` 后按公告日期 `2026-06-05` 与 `2026-06-06` 复扫；CNINFO 代码与名称双检索命中《2025年年度权益分派实施公告》（公告时间 `2026-06-05 20:10:26`），T+1 未检出额外公告；公司官网/IR公开搜索未见新增调研或 H 股审批进展。
- 新增硬事件：公司以现有总股本 `779,160,781` 股为基数，每 10 股派现金 `7.00` 元（含税）并每 10 股转增 4 股，现金分红合计 `545,412,546.70` 元，转增后总股本 `1,090,825,093` 股；股权登记日 `2026-06-11`，除权除息日、现金红利发放日和转增股份上市日均为 `2026-06-12`。
- 交易事件：2026-06-05 龙虎榜与大宗交易按 SZSE/Eastmoney/同花顺/搜狐口径交叉检查，未检出该日新增上榜或大宗交易；SZSE 交易公开信息接口本次返回 500/超时，作为来源缺口保留。
- open-web fallback：Chrome/Grok 因 native host registry key 缺失不可用；普通网页近 24 小时结果主要为权益分派同步、行情/板块讨论和既有 CPO/1.6T/FAU/ELS 叙事重复，全部仅作 `observation_only`。
- 状态变化：追加权益分派实施作为官方公司行动事件；核心基本面判断不变，继续跟踪 CPO 配套交付量化、1.6T 光引擎收入/毛利率、泰国产能、关键物料、客户集中和 H 股审批节点。

## 2026-06-15 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-15` 与 `2026-06-16` 的 T/T+1 检查；CNINFO 公司名/代码查询未发现新增公告、IR 或 H 股审批进展。
- 龙虎榜：2026-06-15 未发现新增龙虎榜。
- 大宗交易：深交所权益类证券大宗交易确认 2026-06-15 发生 1 笔协议交易，成交价 305.04 元、成交 2.00 万股、成交金额 610.08 万元，买卖双方均为机构专用，成交价与当日收盘价一致。
- open-web：普通联网搜索主要返回大宗交易、转增股上市和既有 1.6T/CPO/H 股叙事镜像，没有新增可写入经营层证据的客户、订单、收入占比、毛利率、泰国产能或 H 股审批信号。
- 状态变化：追加小额平价机构大宗交易；核心判断不变，继续跟踪 1.6T 光引擎收入与毛利率、CPO 配套 FAU/ELS、泰国产能、关键物料、客户集中和 H 股发行审批。

## 2026-09-05 日更（20:00 前）

- CNINFO 回查 9 月 1 日《2026年限制性股票激励计划内幕信息知情人买卖公司股票情况的自查报告》：自查未发现内幕信息知情人在本计划首次公开披露前六个月买卖公司股票的情形。该为激励治理程序，未新增经营 event。
- 2026-09-04 SZSE 龙虎榜和大宗交易官方逐公司查询均为 recordcount=0，annList HTTP 500 保留 source_gap。
- 相对基线：实际授予、费用确认及 1.6T/CPO 客户、订单、收入、毛利率仍待核验，`thesis_effect=unchanged`；晚间复扫待完成。

## 2026-09-03 晚间日更

- 公告窗口：CNINFO 公司名查询为 0；SZSE annList 返回 HTTP 500，作为 source_gap 保留。
- 龙虎榜/大宗交易：SZSE ShowReport 对 9 月 3 日逐公司查询均为 recordcount=0。
- 联网观察：仅重复 CPO 配套 FAU/ELS 稳定交付的既有口径，未出现新增客户、订单、1.6T/CPO 独立收入或毛利率事实；thesis_effect=unchanged。

## 2026-08-31 晚间日更

- 公告窗口：北京时间 20:00 后完成 `2026-08-30`、`2026-08-31` 与可访问的 `2026-09-01` T/T+1 回扫。CNINFO 确认第二次临时股东会决议及后续内幕信息知情人自查报告。
- 股权激励：股东会已通过 2026 年限制性股票激励计划、实施考核管理办法及授权事项；这推进了此前草案的审批，但实际授予、归属、费用确认和业绩考核目标实现均待后续披露。
- 龙虎榜 / 大宗交易：二级查询返回 `code=9201` 空结果，不等于交易所官方零记录。
- 相对基线：激励审批不构成 1.6T/CPO 或高速光器件客户、订单、收入、毛利率的新增硬证据，`thesis_effect=unchanged`。

## 2026-08-21 日更（补录 2026-08-19 正式半年报）

- 当前公告窗口：北京时间 20:00 后核验 `2026-08-21` 与 `2026-08-22`；CNINFO 为 0、深交所 annList 返回 HTTP 500；龙虎榜和大宗交易二级查询均为 0。
- 补录原因：逐公司联网核验发现 2026-08-19 已披露但未进入本地状态的半年度报告，现以 CNINFO 原文补入，日期不追溯改写为本轮新公告。
- 半年报：公司披露营收 28.28 亿元、归母净利润 12.04 亿元、扣非归母净利润 11.86 亿元、经营现金流 10.01 亿元，同比分别增长 15.15%、33.92%、36.76%、64.57%。无源光器件收入 15.30 亿元、同比增长 77.40%、毛利率 71.90%；有源光器件收入 12.60 亿元、同比下降 19.57%、毛利率 47.03%。
- 经营边界：报告确认个别物料短缺限制有源器件提产；泰国无源和有源产线稳步提产、江西二期生产楼投入使用。应收账款和存货较年末上升，客户、CPO/1.6T 订单及分项收入仍未披露。
- 资本配置：董事会拟每 10 股派发现金红利 5 元，尚待股东会审议。相对基线，`TFC-01` 与 `TFC-03` 获正式增强，`TFC-02` 仍缺订单、收入和毛利率量化。

## 2026-08-12 日更

- 公告窗口：北京时间 20:00 后完成公告日期 `2026-08-12` 与 `2026-08-13` 的 T/T+1 复扫。CNINFO 命中审计机构、股权激励等 10 份公告；深交所 annList 返回 HTTP 500，保留为来源缺口。
- 治理：公司拟聘任安永华明为 2026 年度审计机构以替代连续服务 16 年的公证天业，管理层所述原因为海外业务、境外子公司及跨境关联交易增加；仍待股东会审议，不能据此推导客户、订单或收入。[公告](https://static.cninfo.com.cn/finalpage/2026-08-13/1225470838.PDF)
- 激励：披露 2026 年限制性股票激励计划草案摘要，拟向 745 人授予 228.87 万股，设置 2027—2029 年扣非净利润考核目标并预计股份支付费用 28,064.95 万元。草案及业绩目标均非已实现经营成果，仍待股东会审议。[公告](https://static.cninfo.com.cn/finalpage/2026-08-13/1225470829.PDF)
- 龙虎榜与大宗交易：东方财富逐家公司 8 月 12 日查询未命中。
- 相对基线：治理和激励计划新增；1.6T/CPO、高速光器件客户、订单、收入、毛利率主线没有新增硬证据，`thesis_effect=unchanged`。

## 2026-07-20 日更

- 公告窗口：检查 2026-07-17 至 2026-07-21。7 月 18 日半年度业绩预告预计归母净利润 11.2405 亿—13.0390 亿元，同比增长 25%—45%；扣非净利润 10.8905 亿—12.8390 亿元，同比增长 25.56%—48.02%。
- 经营驱动与约束：公司将增长归因于全球 AI 和数据中心建设带动高速光器件需求，垂直整合、自动化和精益改善；个别物料紧缺限制提产，汇兑损失增加财务费用，股权激励税前扣除形成正向影响，预计非经常性损益 2,000 万—3,500 万元。
- 龙虎榜 / 大宗交易：7 月 17、20 日均未命中。
- open-web：除业绩预告转述和板块行情外，无新增可靠公司级事实。
- 状态变化：利润增长与高速光器件需求的公司级证据增强，同时物料和汇率约束得到正式确认；仍缺 1.6T/CPO 分产品收入、客户结构、订单、产能利用率和毛利率量化。

## 2026-06-23 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-23` 与 `2026-06-24` 的 T/T+1 检查；CNINFO 公司名/代码查询未发现新增公告、IR 或 H 股审批进展。
- 龙虎榜：2026-06-23 未发现新增龙虎榜。
- 大宗交易：深交所权益类证券大宗交易确认新增 1 笔协议交易，成交价 311.06 元、成交 1.50 万股、成交金额 466.59 万元，买方为中信证券股份有限公司上海分公司，卖方为机构专用；东方财富交叉显示成交价与当日收盘价一致。
- open-web：普通联网搜索主要返回光通信板块和既有 CPO/1.6T 叙事，未发现 CPO 配套 FAU/ELS、1.6T 光引擎、泰国产能、关键物料、客户集中或 H 股发行审批新增硬证据。
- 状态变化：追加小额平价机构大宗交易；核心判断不变，继续跟踪 1.6T 光引擎收入与毛利率、CPO 配套 FAU/ELS、泰国产能、关键物料、客户集中和 H 股发行审批。
