# 603738.SH 泰晶科技 跟踪状态

baseline_status: done
last_checked: 2026-05-28 Asia/Shanghai
browser_scope: chrome_separate_grok_x_tab
grok_status: unavailable_no_callable_chrome_or_grok_tool
open_web_fallback_status: searched_observation_only_no_new_hard_signal
not_investment_advice: true

## 当前跟踪结论

泰晶科技按“石英晶体频率元器件龙头 + 超高基频/有源晶振 + AI 数据中心/光模块时钟链 + 车规晶振国产替代”跟踪。硬证据支持公司有 100MHz 至 625MHz 高基频晶振量产能力、312.5MHz 产品客户验证导入、625MHz 典型低抖动指标、车规专线和 2026Q1 盈利修复；但不能把头部光模块客户、英伟达供应链、订单排产、预付款锁产能或日本全面禁运写成公司确认事实。

基本面质量中高，业绩弹性中高，交易弹性高。后续重点看 312.5MHz/625MHz 从推广验证到批量收入的进展、Q2/Q3 毛利率和扣非利润延续性、BAW Q4 小批量样品、扩产项目与真实订单匹配度、车规客户认证和收入占比。

## 最新硬事件

- 2026-05-23：新增为 watchlist enabled 公司并完成首次基线；公告窗口检查 2026-05-23 与 2026-05-24 未发现新增公告；2026-05-22 未发现龙虎榜或大宗交易。
- 2026-05-20：新浪公告镜像披露《2026年5月19日投资者关系活动记录表》。公司称 312.5MHz 处于市场拓展与客户验证导入中；625MHz 是下一代对应单波 400G 的技术方案，典型相位抖动低至 15fs；光模块高频差分晶振处于市场推广期、收入占比较小；BAW 振荡器研发有序推进，预计 Q4 可小批量并提供样品。
- 2026-04-30：2026Q1 营业收入 2.492 亿元，同比 +24.31%；归母净利润 2,304.45 万元，同比 +165.22%；扣非归母净利润 1,529.01 万元，同比大幅修复；经营活动现金流净额 6,284.87 万元，同比 +125.41%。
- 2026-04-23：2025 年报披露收入 9.437 亿元，同比 +14.94%；归母净利润 5,222.44 万元，同比 -40.37%；主营业务毛利率 18.30%，同比下降 4.99 个百分点；公司称市场环境趋紧导致产品价格下行、股权激励费用增加压制利润。

## 2026-05-24 日更

- 公告窗口：北京时间 20:00 后复核公告日期 `2026-05-24` 和 `2026-05-25`，CNINFO 与上交所公告窗口未发现新增公司公告；公司网站未发现新的有日期公告或投资者关系记录。
- 龙虎榜：二级交易数据未发现 `2026-05-22` 新增龙虎榜记录，最近可见上榜仍为 `2026-05-08`。
- 大宗交易：总控补查东方财富 `RPT_DATA_BLOCKTRADE` 与 `RPT_BLOCKTRADE_STA`，`2026-05-22` 返回空数据，未发现新增大宗交易。
- Grok/open-web：Chrome/Grok 独立公司检索未发现过去 24 小时新增硬信号；总控 open-web fallback 只发现行情、官网和既有 312.5MHz/625MHz 叙事延续，不作为确认事实。
- 状态判断：本轮没有新增 official_announcement、investor_relations、trading_event_lhb 或 trading_event_block_trade；核心基线不变，继续等待客户、订单、收入占比、产能、价格和毛利率量化。

## 2026-05-25 日更

- 公告窗口：北京时间 20:00 后执行，覆盖 `2026-05-25` 和 `2026-05-26`；上交所公告接口未返回新增公司公告。CNINFO 定向查询窗口未见新增 603738 记录，但因后续校验被中断，保留 `source_gap`，不把 CNINFO 完整性写成最终确认。
- 龙虎榜：东方财富龙虎榜明细接口最近记录仍为 `2026-05-08`，未发现 `2026-05-25` 新增龙虎榜。
- 大宗交易：东方财富 `RPT_DATA_BLOCKTRADE` 与 `RPT_BLOCKTRADE_STA` 均确认 `2026-05-25` 新增 1 笔大宗交易，成交价 48.20 元，成交量 202,500 股，成交金额 976.05 万元，较收盘价 49.19 元折价约 2.01%；买方为广发证券广州中山三路中华广场营业部，卖方为西南证券浏阳环府路营业部。
- Grok/X：Chrome 插件可连接；Grok.com 打开后触发 Cloudflare blocked 页面；X 独立标签以 `"泰晶科技" OR "603738" since:2026-05-24 until:2026-05-26` 检索未返回公司相关结果。该层未形成新增硬信号。
- 状态判断：新增交易结构事件，金额约 976 万元、占流通股比例约 0.052%，偏小；不改变基本面主线，但需要观察是否连续出现折价大宗交易、减持压力或机构承接。

## 2026-05-28 日更

- 公告窗口：北京时间 13:40、20:00 前执行；CNINFO `hisAnnouncement/query` 和上交所 `queryCompanyBulletin.do` 检索 `2026-05-26` 至 `2026-05-28` 均返回 0 条新增公告。公司官网首页最新新闻仍为 `2026-05-22`，投资者关系入口跳转上交所公告页，未发现同窗口新增正式 IR/公告。`announcement_window_checked=pending_evening_rescan`。
- 龙虎榜：东方财富 `RPT_DAILYBILLBOARD_DETAILSNEW` 按 `603738`、`2026-05-26` 至 `2026-05-27` 返回空数据；`2026-05-28` 当前未收盘且最终龙虎榜数据不可用，未作为完成交易日入账。
- 大宗交易：东方财富 `RPT_DATA_BLOCKTRADE` 与 `RPT_BLOCKTRADE_STA` 按 `603738`、`2026-05-26` 至 `2026-05-27` 返回空数据；`2026-05-28` 当前未收盘且最终大宗交易数据不可用，未作为完成交易日入账。
- Grok/open-web：当前无可调用 Chrome/Grok 工具，标记 `grok_status=unavailable_no_callable_chrome_or_grok_tool`；普通 open-web fallback 只发现既有 OFC/625MHz/312.5MHz 叙事、行情页和二级讨论，未发现新增官方客户、订单、产能、价格或收入占比硬披露，按 `observation_only` 处理。
- 状态判断：本轮没有新增 official_announcement、investor_relations、dragon_tiger 或 block_trade material event，`events.jsonl` 不追加；核心验证问题维持不变，等待 20:00 后 T/T+1 公告窗口复扫。

## Source gaps

- 312.5MHz/625MHz 缺实际出货量、客户、订单、收入占比、产能和毛利率量化。
- 光模块业务仍被公司表述为市场推广期，不能确认已经进入中际旭创、新易盛、英伟达等供应链。
- 6 亿元扩产仅有问答和备案/传闻层信息，缺正式项目公告与订单匹配证据。
- BAW 振荡器为 Q4 小批量样品预期，尚未验证客户和商业化节奏。
- 日本高端晶振管制相关信息未在本轮得到官方法律/监管原文确认。
- CNINFO `2026-05-25`/`2026-05-26` 完整性校验被本轮 bounded closeout 中断，后续 controller 可复查。

## 后续跟踪问题

1. 312.5MHz 是否从市场推广和客户验证导入转为批量供货，并披露收入、客户、订单或产能利用率。
2. 625MHz 是否出现小批量/量产、客户验证、价格和毛利率证据。
3. BAW 振荡器 Q4 小批量和样品能否按期兑现，是否对应明确客户验证。
4. 2026Q2/Q3 毛利率、扣非利润和经营现金流能否延续 Q1 修复。
5. 6 亿元扩产相关备案、项目、设备和产能安排是否披露，是否对应真实订单需求。
6. 车规有源/无源/RTC 是否披露 TOP10 Tier1 或主机厂客户进展、收入占比和认证阶段。

## 2026-05-29 日更

核查窗口：北京时间 20:00 后运行，公告硬门覆盖公告日期 `2026-05-29` 与 `2026-05-30`，`announcement_window_checked=T_and_T_plus_1`。

- 公告 / CNINFO / 交易所披露：CNINFO 与上交所公司公告同窗口均未发现新增正式公告。公司官网 2026-05-29 发布“看25MHz晶振，如何支撑以太网：高速运转”教育文章，属于官网内容但不是投资者关系披露、客户/订单或经营硬信号。
- 龙虎榜：上交所交易公开信息确认泰晶科技 2026-05-29 因“日价格振幅达到15%的前五只证券”上榜，当日振幅 17.61%，成交量 56,652,812 股，成交额 25.87 亿元；买入前五合计约 2.99 亿元，卖出前五合计约 3.15 亿元，前五净卖出约 1,604.45 万元，沪股通专用同时位列买一和卖一。东方财富龙虎榜数据与官方总额交叉一致。
- 大宗交易：2026-05-29 未发现泰晶科技新增大宗交易记录。
- Grok/X：当前工具面没有可调用 Chrome/Grok/@chrome，记录为 `unavailable_no_callable_chrome_tool`；未使用 Browser Use 或 Playwright 冒充登录态 Chrome/Grok。
- open-web fallback：普通公开网页主要为行情页、股吧盘中情绪和既有 AI 光模块晶振叙事，未发现新增客户、订单、312.5MHz/625MHz放量、产能、毛利率或收入占比硬证据。
- 本次状态变化：追加龙虎榜交易结构事件。基本面基线不变，继续跟踪 312.5MHz/625MHz 客户、订单、收入占比或产能利用率正式披露，以及 5/29 龙虎榜净卖出是否只是高波动换手还是后续交易结构压力。

## 2026-05-30 日更状态
- 检查窗口：公告 T/T+1（2026-05-30 至 2026-05-31）、最近交易日龙虎榜/大宗交易（2026-05-29）。
- 结论：未发现新的T/T+1公告或大宗交易；5月29日龙虎榜已记录，继续观察换手降温及312.5/625MHz高频晶体客户验证。
- 来源分层：官方披露/交易数据优先；Grok/X不可调用时使用 open-web fallback，相关线索仅作观察池。
## 2026-06-01 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-01` 与 `2026-06-02` 的 T/T+1 检查；CNINFO、上交所、东方财富公告镜像未发现新增正式公告。公司官网 2026-06-01 可见晶体测试科普文章，不属于 IR/公告或经营硬信号。
- 龙虎榜：2026-06-01 未发现新增龙虎榜，最近已记录龙虎榜仍为 2026-05-29。
- 大宗交易：东方财富大宗交易数据确认 2026-06-01 发生 1 笔平价大宗交易，成交价 42.40 元，成交 20.50 万股，成交额 869.20 万元；买方华福证券上海浦明路，卖方华泰证券天津东丽开发区二纬路。
- Grok/open-web：当前无可调用 Chrome/Grok 工具，未使用 Browser/Playwright 替代；open-web fallback 仅见光模块/高频晶振叙事和二级讨论，无官方客户、订单、收入占比、产能或价格确认。
- 状态变化：新增小额平价大宗交易；基本面跟踪主线不变，继续等待 312.5MHz/625MHz 客户验证转批量、收入占比、产能利用率和毛利率量化证据。
