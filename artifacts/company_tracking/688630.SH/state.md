...
| 字段 | 当前判断 |
|---|---|
| terminal demand | AI 训练/推理算力、GPU/ASIC/HBM 多芯片集成、高端封装载板、类 CoWoS-L / CoPoS / CoWoP 等先进封装路线。 |
| chain node | `CoWoS / EFB / SoIC / advanced packaging / substrate` 的上游设备观察层，重点是 WLP/PLP 直写光刻、IC 载板曝光、RDL/Bumping/TSV 制程。 |
| company exposure | WLP 2000 / WLP 2000P、PLP 3000 / PLP 4000、MAS 6P、NEX 30 等直写光刻与 IC 载板设备。 |
| pass-through path | AI 芯片先进封装和高阶封装载板需求 -> 头部封装/载板客户扩产与工艺升级 -> 直写光刻设备订单、出货、验收 -> 收入、毛利和现金流兑现。 |
| 当前分层 | `advanced_packaging_equipment_watch / evidence_gap_card`；可跟踪，不升 `main_candidate`。 |
| 不能上调原因 | 缺 HBM4/CoWoS 直接客户名单、订单金额、收入占比、毛利率、验收收入、交付节奏和客户排产受限证据。 |
...
| 2026-03-14 | A | 芯碁微装 2025 年年度报告摘要 / CNINFO | WLP 系列用于 12/8 inch 集成电路先进封装，覆盖 Flip Chip、Fan-In/Out WLP、2.5D/3D，并在 RDL、Bumping、TSV、SOW 等制程具备优势；PLP 系列覆盖 FC CSP、FC BGA、Fan-In/Out PLP、2.5D/3D。证明产品覆盖，不证明特定 HBM4/CoWoS 客户。 |
| 2025-08-28 | A | 芯碁微装 2025 年半年度报告 / CNINFO | MAS 6P 在封装载板头部客户验收并投入量产、获得批量订单；WLP 2000 获得中道头部客户重复订单并出货，WLP 系列在多个头部客户验收量产中。证明设备订单和量产客户硬证据，但客户未实名、金额/收入占比/交付节奏 N/A。 |
| 2026-01-12 | B | 第一财经/证券时报等转述芯碁微装官微 | WLP 系列已助力多家先进封装头部厂商实现类 CoWoS-L 产品量产，预计 2026H2 量产爬坡；WLP 在手订单突破 1 亿元。因未直接读到官微原文且非交易所公告，按 B/公司官微媒体转述处理。 |
| 2026-06-21 | repo reference | [寡头节点研究](../../../docs/ai_chain_disassembly/oligopoly_node_research.md) | 严格 `CoWoS` 是 TSMC 体系；宽口径 2.5D/3D advanced packaging 供应商较多。不能把“类 CoWoS-L”与 TSMC CoWoS 主供应链、HBM4 封装客户直接等同。 |

## 三层判断

| 层级 | 当前判断 | 依据与限制 |
|---|---|---|
| 基本面质量 | 中高，设备平台观察 | 公司有 PCB、IC 载板、WLP/PLP 直写光刻设备平台，先进封装和载板客户验证已有 A 级披露；但先进封装收入、客户集中度、毛利率和长期份额仍缺拆分。 |
| 业绩弹性 | 中高，条件式 | 若 WLP/PLP 与 IC 载板设备订单持续转出货、验收和回款，且 2026H1/H2 披露先进封装收入占比提升，业绩弹性可继续上修。当前 HBM4/CoWoS 直接订单、交付节奏、毛利率均为 N/A。 |
| 交易弹性 | 高，需防主题过拟合 | 邮件与媒体线索容易触发“先进封装设备国产替代”交易弹性；但交易弹性不能反推 SK 海力士、TSMC、Amkor 或 NVIDIA/AMD 供应链确认。 |
...
1. 公司是否披露 WLP/PLP/IC 载板设备分产品收入、毛利率、合同负债、订单金额和验收收入。
2. 是否出现 HBM4 / CoWoS / CoWoS-L / CoPoS / CoWoP 相关客户、订单、PO、验收、交付排期或收入占比。
...
- 第一财经转述芯碁微装官微 WLP 订单线索: https://www.yicai.com/news/102999757.html
- 证券时报一季度订单与先进封装设备可见度报道: https://www.stcn.com/article/detail/3871582.html
