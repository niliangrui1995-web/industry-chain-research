# 长飞光纤（601869.SH）滚动跟踪状态

更新日期：2026-07-15
跟踪性质：公司首次深研后的滚动状态，不构成买卖建议。
grok_status: not_used_by_policy

## 当前跟踪结论

长飞光纤当前处在“传统光纤周期修复 + AI 光联接叙事强化 + 海外扩张提速”的叠加阶段。硬证据最强的是 2026Q1 收入、毛利率和净利润明显改善；第二层证据是 2025 年光互联组件收入高增；第三层是空芯光纤和 AI-2030 的技术/试点进展。最需要警惕的是交易预期已经明显领先，空芯光纤和数据中心相关新型光纤尚未形成可量化的大规模业绩贡献。

## 2026-07-15 日更

- 公告窗口：北京时间 20:00 后完成 2026-07-14、2026-07-15 与 T+1 2026-07-16 检查；CNINFO 确认 7 月 15 日新增《2026 年半年度业绩预增公告》。
- 新增官方业绩事件：公司预计 2026H1 归母净利润 24 亿至 30 亿元，同比增长 711% 至 914%；扣非归母净利润 20 亿至 26 亿元，同比增长 1,349% 至 1,784%。公司将增长归因于算力数据中心建设提速、新型光纤光缆需求增长和供需改善，并明确提到算力数据中心相关业务拓展、核心客户突破、产品结构优化和盈利能力提升。预告未经审计。
- 龙虎榜 / 大宗交易：2026-07-14 至 2026-07-15 未发现新增龙虎榜或大宗交易。
- 状态变化：相对“AI/新型光纤仍缺规模收入证明”的基线，官方预告已经把算力数据中心业务、核心客户拓展与利润增长正式关联，经营证据等级显著上升；仍缺客户名称、分产品收入、ASP、毛利率和现金流拆分，空芯光纤也不能据此直接视为利润主因。

## 2026-07-10 日更
- 公告窗口：北京时间 20:00 后完成 `2026-07-10` 与 `2026-07-11` 的 T/T+1 检查；CNINFO 在 T+1 命中 9 份董事会换届、候选人声明、职工代表董事选举、临时股东会通知及会议资料。
- 新增官方治理事件：第四届董事会提名 7 名非独立董事和 4 名独立董事候选人；职工代表大会另选举副总裁王瑞春为第五届董事会职工代表董事，拟共同组成 12 人董事会，任期三年。董事长马杰、总裁庄丹等核心管理层获继续提名，候选人仍需临时股东会审议。
- 龙虎榜 / 大宗交易：2026-07-10 未发现长飞光纤新增龙虎榜或大宗交易；2026-07-09 回扫无新增延迟数据。
- 状态变化：本轮属于治理结构换届，不改变“传统光纤周期修复 + AI 光联接叙事强化 + 海外扩张”主线；仍需用光纤价格、数据中心产品订单、海外收入和毛利率验证经营兑现。

## 2026-07-09 日更
- 公告窗口：北京时间 20:00 后完成 `2026-07-09` 与 `2026-07-10` 的 T/T+1 检查；CNINFO 命中 2026-07-10《5%以上股东减持股份结果公告》。
- 新增正式事件：长江通信本轮集中竞价减持计划时间区间届满，累计减持 999,985 股，占总股本 0.12%，减持总金额 41,207.61 万元；当前持股 14.23%，计划仅剩 15 股未完成。
- 龙虎榜/大宗交易：2026-07-09 未发现长飞光纤新增龙虎榜或大宗交易。
- 状态变化：这是股东减持窗口收口事件，边际降低该计划不确定性；不改变“传统光纤周期修复 + AI光连接叙事强化 + 海外扩张”主线，也不提供数据中心光纤订单、ASP、收入或毛利率新增证据。

## 2026-06-30 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-30` 与 `2026-07-01` 的 T/T+1 检查；CNINFO 公司名/代码查询确认 T+1 新增年度股东会决议及法律意见书。
- 新增官方事件：2025 年年度股东会审议通过 2025 年度利润分配方案、2026 年度审计机构、2026 年度对外担保额度、资产池、外汇及原材料套期保值等议案；会议无否决议案。
- 龙虎榜 / 大宗交易：2026-06-30 未发现长飞光纤新增龙虎榜或大宗交易。
- 状态变化：本次偏例行治理/分红/授权事项，不改变“传统光纤周期修复 + AI 光联接叙事强化 + 海外扩张提速”的主线；仍需用 Q2/Q3 财报、光纤价格、光互联组件、海外收入和毛利率验证 AI 数据中心收入传导。

## 关键假设

1. 普通光纤价格自低位修复可以至少维持数个季度，而不是短期急单价格扰动。
2. 公司棒-纤-缆一体化和预制棒能力能把价格上涨转化为毛利率提升。
3. 光互联组件业务继续增长，成为比空芯光纤更快兑现的 AI 相关收入来源。
4. 海外基地和 H 股配售资金能带来海外业务持续增长，而不是仅提升收入规模。
5. 空芯光纤从试点到规模商用仍需时间，短期不把它作为利润主因。

## 最新证据

- 2026Q1：收入 36.95 亿元，同比 +27.70%；归母净利润 4.95 亿元，同比 +226.40%；扣非归母净利润 4.61 亿元，同比 +966.44%；经营现金流净额 6.18 亿元，同比 +41.85%。
- 2026Q1：利润高增的公司解释是营业收入和毛利率同比增加；按利润表测算，Q1 毛利率约 41.5%。
- 2025 年：总收入 142.52 亿元，同比 +16.85%；归母净利润 8.14 亿元，同比 +20.40%；经营现金流净额 36.53 亿元，同比 +104.83%。
- 2025 年：光传输产品收入约 83.46 亿元，同比 +6.09%，毛利率 35.90%；光互联组件收入约 31.44 亿元，同比 +48.58%，毛利率 39.73%。
- 2025 年 12 月 IR：公司称空芯光纤仍处发展前期，相关业务尚未对经营业绩产生重大影响。
- 2026 年 2 月异动公告：公司提示数据中心内部及互连相关光纤光缆产品数量占全球需求总量比例较小，公司目前不从事 CPO 相关业务。
- 2026-05-08 腾讯行情：601869.SH 收盘价 397.23 元，换手率 4.79%，总市值约 3,288.69 亿元，TTM PE 约 284.20。仅作为交易弹性观察。
- 2026-06-28 窄口径补证：本轮“光纤上游”涨价/紧张最明确落在光纤预制棒，尤其数据中心高密度布线相关的 A2/G.657.A2 类光棒；普通 G.652D 光棒也有二级涨价线索，但普通光纤/光缆、光棒、石英材料和高密度数据中心连接件不能混写。长飞侧仍只确认行业上行与部分产品价格波动不确定，尚未闭合到 G.657.A2 实现 ASP、订单、收入拆分或毛利。
- 2026-06-30 窄口径补证：中国移动 2026-2027 年 G.654 光缆产品（高铁用）集采候选人公示确认长飞为第一中标候选人，不含税报价 200,751,151.91 元、拟中标份额 50%；可信行业源按本轮 64.94 万芯公里和加权不含税报价折算约 310.74 元/芯公里，较 2025 年 G.654.E 光纤光缆集采可比折算价约 188.66 元/芯公里上涨约 64.7%。这只能确认运营商 G.654/G.654.E 高端光缆集采价格上行，仍不能升级为长飞收入、订单金额、ASP 或毛利率改善已经闭合。

## 2026-06-22 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-22` 与 `2026-06-23` 的 T/T+1 检查；CNINFO 公司名/代码查询未发现新增发行人公告。
- 龙虎榜 / 大宗交易：东方财富全市场龙虎榜和大宗交易数据未发现 2026-06-22 长飞光纤新增上榜或成交。
- open-web 观察：长江日报大武汉客户端报道，长飞光纤 2026-06-22 A+H 总市值突破 3000 亿元，并把行情驱动归因于 AI 数据中心光纤需求扩张、光纤预制棒扩产周期和光纤产品价格上涨等行业叙事。
- 状态变化：交易弹性和市场关注度继续抬升，但这属于可信媒体/行情观察，不等同于长飞新增客户订单或数据中心光纤收入确认；公司层面仍需用 Q2/Q3 财报、光纤价格、光互联组件、海外收入和毛利率继续验证。

## 2026-06-09 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-09` 与 `2026-06-10` 的 T/T+1 检查；CNINFO、上交所口径未发现长飞光纤新增公司公告或 IR。
- 龙虎榜 / 大宗交易：2026-06-09 未发现长飞光纤新增龙虎榜或大宗交易。
- open-web 观察：Amazon 与 Corning 2026-06-08 官方发布数十亿美元协议，Corning 将供应光纤、光缆及连接解决方案以支持 Amazon 美国数据中心扩张，并扩建北卡相关产能。A 股/港股媒体将该事件映射到光纤板块活跃和长飞光纤涨停。
- 状态变化：外部官方事件增强“全球云厂商锁定数据中心光纤/连接解决方案供给”的行业景气观察，但不是长飞光纤订单、客户或收入确认。公司层面仍需等待光纤价格、光互联组件、空芯/多芯光纤、海外收入和毛利率在公告、IR 或财报中继续验证。

## 2026-05-09 日更

- 公告/披露：核对 2026-05-08~2026-05-09 东方财富公告 np-anotice-stock、CNINFO 历史公告、上交所公司公告查询，均未发现长飞光纤新公告或交易所披露；最近一批 A 股公告仍为 2026-04-30 披露的制度/ESG/董事会等文件。
- 投资者关系：公司官网 IR 页面在当前命令行环境访问被站点拒绝（HTTP 444 / curl 56），未能完成官网页面直连复核；本次以 CNINFO、上交所和东方财富公告接口作为正式披露核对来源。
- 龙虎榜：东方财富 RPT_DAILYBILLBOARD_DETAILSNEW 按 601869、2026-05-08~2026-05-09 查询返回空数据；最近一条历史记录仍为 2026-03-26。
- 大宗交易：东方财富 RPT_DATA_BLOCKTRADE 与 RPT_BLOCKTRADE_STA 按同一日期窗口查询均返回空数据；最近一条历史明细/统计仍为 2025-09-30。
- Grok/X：当前无 @chrome 工具暴露，按任务要求记录为 unavailable_chrome_tool_not_exposed；未用普通网页搜索替代 Grok/X。
- 状态判断：本次无新增正式事件写入 events.jsonl，核心跟踪假设不变；短期继续关注公告/IR 对 Q2 毛利率、光互联组件、空芯光纤规模化节点的补证。

## 观察池

- 光纤价格：G.652D、G.657A2、G.654.E 的集采价格、现货价格、急单价格是否分化。
- 光纤上游拆层：普通光纤/光缆、G.652D 普通光棒、A2/G.657.A2 高密度布线预制棒、高纯石英砂/光纤级石英套管/炉芯管、以及 FAU/MT/MPO/PM fiber/fiber array 等高密度连接/被动件分别跟踪，避免用一个环节替代另一个环节。
- 运营商集采：中国移动、中国电信、中国联通普通光缆、新型光纤、空芯光纤、混合光缆中标份额和价格。
- AI 数据中心：数据中心内部/互连场景光纤光缆产品的收入占比、订单、客户是否披露。
- 光互联组件：长芯博创收入、毛利率、订单质量、客户集中度。
- 海外：海外收入占比、海外基地投产/利用率、H 股配售资金具体投向。
- 空芯光纤：从试点线路到规模招标的时间、单价、良率、标准、运维验证。
- 风险提示：是否继续出现股价异动公告、减持公告、配售/融资稀释、监管问询。

## Source gaps

- 年报全文未完成结构化提取：需补地区收入、产品明细、产能利用率、主要子公司和订单披露。
- 客户证据不足：云厂商、海外运营商、光模块客户名称未见官方明确披露。
- 产能口径不足：光棒、光纤、光缆、空芯光纤名义产能缺官方数字。
- 价格周期缺官方连续数据：已补到中国移动 G.654 高铁用光缆候选报价和可信行业源折算的上次可比价，但该序列仍是运营商集采层，不是长飞公司实现 ASP。
- G.654/G.654.E 集采价缺公司实现口径：本轮长飞第一候选与 50% 拟中标份额尚需后续正式结果、合同/订单金额、履约节奏、收入确认和分产品毛利率验证。
- G.657.A2 价格缺公司实现口径：需补运营商/云厂商采购价格、长飞分产品 ASP、订单或毛利率拆分；当前 550% 涨幅仍是可信二级行业价格线索。
- 石英材料传导缺公司口径：高纯石英砂、光纤级石英套管、炉芯管等只构成上游成本/供给线索，尚缺长飞采购成本、库存、锁价、供应商或毛利率传导披露。
- 高密度连接/被动件未闭合：Corning 长协能证明北美 optical fiber/cable/connectivity 锁产能观察增强，但 FAU、MT ferrule、MPO、PM fiber、fiber array 等单品仍缺交期、涨价、订单排队或长飞收入证据。
- Corning 长单未桥接长飞：Amazon/NVIDIA/Meta 对 Corning 的锁供只能说明北美光连接产能被锁定，不能替代长飞客户、订单、收入或毛利证据。
- 公司官网 IR 页面本次直连失败：需在可访问官网或 Chrome 登录环境下补查 2026-05-08~2026-05-09 是否有投资者服务/公告通函新增。
- Grok/X 未运行：当前无 @chrome 工具暴露，缺少社交热度与海外实时讨论层。

## Next questions

1. 2026Q2 毛利率能否继续高于 2025 年全年水平？
2. Q1 利润中有多少来自普通光纤价格修复，多少来自光互联组件和新型光纤？
3. 公司是否披露 2025 全年海外收入占比及海外利润率？
4. G.657.A2 或其他 bend-insensitive fiber/数据中心光纤是否出现长飞口径的订单、客户、ASP、收入占比或毛利率拆分？
5. 中国移动 G.654 高铁用光缆拟中标是否落到正式结果、长飞合同/订单金额、履约节奏和分产品毛利率？
6. 光棒涨价是否能被运营商/云厂商采购价格、长飞光传输产品 ASP 或 Q2/Q3 毛利率验证，而不是只停留在行业报价？
7. 高纯石英砂、石英套管、炉芯管是否出现官方/公司级供给紧张、锁价、成本传导或长飞采购侧证据？
8. 高密度连接/被动件是否出现 Corning/Fujikura/Sumitomo、模块厂或连接件厂层面的交期、涨价、订单排队和客户认证证据？
9. 空芯光纤下一次可验证的规模化节点是什么：运营商集采、云厂商 DCI、海外试点还是标准推进？
10. 高估值下，股东减持、H 股折价配售和 A/H 溢价会否成为交易压制因素？

## 常用来源

- 年报摘要：https://static.cninfo.com.cn/finalpage/2026-03-28/1225046910.PDF
- 一季报：https://dataclouds.cninfo.com.cn/shgonggao/hsomarket/2026/20260429/3a732306ab82491fa3649fb0b078ab0f.PDF
- 公司官网：https://en.yofc.com/list/164.html
- AI-2030：https://en.yofc.com/view/3398.html
- IR 2025-12-26：https://dataclouds.cninfo.com.cn/shgonggao/investor/2025/20251226/2b3b93d44dc44df196884c97afa28810.PDF
- IR 2025-06-27：https://dataclouds.cninfo.com.cn/shgonggao/2025/2025-06-27/b40a18cc533a11f089d0fa163e39923a.pdf
- 异动公告 2026-06-24：https://paper.cnstock.com/html/2026-06/24/content_2234332.htm
- 央视财经光纤光缆订单/光棒瓶颈：https://www.21jingji.com/article/20260604/herald/b04acaaff318ab90a5827e23ee479460.html
- 上证报/证券时报光棒涨价：https://www.stcn.com/article/detail/3973253.html
- 上证报高纯石英材料传导：https://www.21jingji.com/article/20260605/herald/1db25ddc038887160e7178f60674eafa.html
- Amazon-Corning：https://www.aboutamazon.com/news/company-news/amazon-corning-fiber-optics-1000-jobs-north-carolina
- NVIDIA-Corning：https://nvidianews.nvidia.com/news/nvidia-and-corning-announce-long-term-partnership-to-strengthen-us-manufacturing-for-ai-infrastructure
- Meta-Corning：https://about.fb.com/news/2026/01/meta-6-billion-agreement-corning-support-us-manufacturing/
- G.657.A2 涨价二级来源：https://paper.cnstock.com/html/2026-06/23/content_2234171.htm
- 中国移动 G.654 高铁用光缆候选人公示：https://b2b.10086.cn/#/noticeDetail?publishId=2070428105391411202&publishUuid=f86db09e49654538a23c9a5365ebde99&publishType=PROCUREMENT&publishOneType=CANDIDATE_PUBLICITY
- G.654 集采价格折算二级来源：https://webadmin.chinacable.com/news/details/11315
- 2025 G.654.E 集采候选价二级来源：https://webadmin.chinacable.com/news/details/9894

## 2026-05-09 主控补充：open_web_fallback

- 触发原因：`@chrome` / Grok 当前工具未暴露，按更新后的自动化规则补做 Codex open-web fallback。
- fallback 状态：`open_web_fallback_status=searched/secondary_research_only`。
- 观察结果：普通公开网页检索未发现 2026-05-08 至 2026-05-09 新公告、龙虎榜或大宗交易；发现近期券商/财经平台继续讨论数据中心光纤利润弹性，但属于二级研究观点，不是公司新披露。
- 证据边界：该层不是 Grok/X、不是 Chrome 登录态结果；二级研究只进观察池，不改变基线。

## 2026-05-09 worker batch_no=1 复核

- worker_scope：仅复核 `601869.SH`，未改动 watchlist Excel、日报 `artifacts/company_tracking/2026-05-09.md` 或 `run_status.md`。
- browser_scope：`not_available`。
- grok_status：`unavailable: chrome plugin tool not exposed`。
- open_web_fallback_status：`searched/secondary_research_only`；普通联网搜索仅作为观察池，不作为确认事实。
- 公告/交易所披露：东方财富公告 `np-anotice-stock`、CNINFO `hisAnnouncement/query` 按 2026-05-08 至 2026-05-09 查询均返回 0；上交所公告接口本轮命令行直连未成功返回可解析 JSON，因此以上交所公开检索和东方财富/CNINFO 交叉结果判定未发现本窗口新增正式公告。
- 龙虎榜：东方财富 `RPT_DAILYBILLBOARD_DETAILSNEW` 按 `SECURITY_CODE=601869`、2026-05-08 至 2026-05-09 查询返回 0，未发现本窗口龙虎榜。
- 大宗交易：东方财富 `RPT_DATA_BLOCKTRADE` 与 `RPT_BLOCKTRADE_STA` 按同一窗口查询均返回 0，未发现本窗口大宗交易。
- 状态变化：`state_updated_no_event`；未发现需要追加 `events.jsonl` 的新公告、龙虎榜、大宗交易或已验证重大观察。
- 风险备注：公司官网 IR 中文页面本轮仍存在直连失败/连接中断风险；open web 中的价格弹性、数据中心需求讨论多为二级研究或行情页面，应继续放在观察池，不能替代公司公告、交易所披露或后续财报验证。

## 2026-05-13 worker batch_no=1 复核

### 独立任务块

- company：长飞光纤光缆股份有限公司
- ticker：`601869.SH`
- aliases：长飞光纤、长飞光纤光缆、YOFC、06869.HK
- tracking_focus：光纤价格周期、AI 光联接/数据中心、光互联组件、空芯光纤、海外扩张、交易拥挤度。
- browser_scope：`fallback_no_browser`
- grok_status：`unavailable_chrome_plugin_not_exposed`
- open_web_fallback_status：`searched/no_new_hard_event`

### 完成清单

- baseline_read：yes；本轮基线已 done，未写 `baseline.md`。
- state_read：yes。
- events_read：yes；未发现重复或需要补写的硬事件。
- announcements_checked：yes；CNINFO `hisAnnouncement/query` 按 2026-05-10 至 2026-05-13 返回 0，东方财富公告日期窗查询返回 0；公开检索未发现 2026-05-13 新正式公告。
- investor_relations_checked：partial；公司官网 IR 页面命令行直连仍出现连接意外关闭，需后续在可访问官网或 Chrome 登录环境下补查。
- hkex_checked：partial；HKEX 页面可达，但本轮 open-web 未发现 06869.HK 在 2026-05-13 的新增公告信号。
- lhb_checked：yes；东方财富龙虎榜接口按 2026-05-10 至 2026-05-13 查询返回空，最近历史龙虎榜仍为 2026-03-26。
- block_trade_checked：yes；东方财富大宗交易接口按 2026-05-10 至 2026-05-13 查询返回空，最近历史大宗交易仍为 2025-09-30。
- grok_checked：no；Chrome/Grok 工具面未暴露，未使用 Browser Use/Playwright 替代登录态。
- open_web_fallback_checked：yes；普通公开网页仅发现 2026-05-12 收盘后的资金流、行情和二级研究延续讨论，属于 open-web fallback 观察层，不作为 Grok/X 或公司确认事实。
- events_appended：no；无新增 official_announcement、trading_event_lhb、trading_event_block_trade、investor_relations、trusted_media 硬事件。
- state_updated：yes；仅记录本轮来源边界、完成清单和 source gap。

### 状态判断

本轮未改变核心假设：长飞光纤仍处在光纤周期修复、AI 光联接叙事和海外扩张共同驱动的高预期阶段。2026-05-13 的新增公开信号主要是行情/资金流与二级研究延续，不构成公司新披露或交易所交易事件。短期继续把 2026Q2 毛利率、光互联组件增长、空芯光纤规模化节点、海外收入/利润质量作为下一轮硬验证项。

### 本轮 source gaps / miss risk

- 公司官网 IR 页面仍有命令行直连失败风险，可能漏掉官网投资者服务栏目的非交易所同步内容。
- HKEX 仅做 open-web 信号复核，未完成结构化公告列表抓取；若 H 股端先发通函，需后续补查。
- Chrome/Grok 未暴露，缺少登录态 Grok/X 对海外讨论、社交热度和非中文线索的发现层。
- 普通 open-web fallback 不能替代 Grok/X，也不能替代公告、交易所披露和公司 IR。

## 2026-05-19 batch_no=1 补齐复核

- 公告窗口：已在北京时间 20:00 后补扫公告日期 `2026-05-19` 和 `2026-05-20`，completion 记录 `announcement_window_checked=T_and_T_plus_1`。上交所/CNINFO 确认 2026-05-19 业绩说明会预告公告，未发现 2026-05-20 T+1 新公告。
- 新增硬事件：2026-05-19 上交所公告确认公司将于 2026-05-22 15:00-16:00召开2025年度暨2026年第一季度业绩说明会。
- 投资含义：这是正式 IR 沟通窗口，不直接改变经营结论；会后重点追踪光纤价格、数据中心需求、海外扩张、特种/空芯光纤和毛利率问答。
- 交易事件：2026-05-19 未发现新增龙虎榜或大宗交易记录。
- open-web 观察：二级媒体继续讨论全球光棒紧缺、光纤涨价和 AI 光连接需求，但该层仅作为观察池，不能替代公司公告、财报或 IR 问答验证。

## 2026-05-22 业绩说明会复扫

- 公告窗口：北京时间 20:00 后检查公告日期 `2026-05-22` 和 `2026-05-23`，CNINFO/SSE 未见新增正式公告；中国证券网路演中心已披露 2025 年度暨 2026 年第一季度网上业绩说明会互动内容。
- 新增 IR 要点：公司称面向智算中心提供 Scale-up、Scale-out、Scale-across 三类光联接产品方案；空芯光纤连续传输距离突破 91 公里、最低衰减 0.04dB/km，全球参与 13 个试验网项目并开通商用线路超万公里；多芯光纤具备批量制备能力；G.654.E 已超过百万芯公里规模部署；2025 年海外收入约 60.92 亿元、占总收入 42.7%。
- 状态变化：数据中心光联接、空芯/多芯/G.654.E 和海外收入占比证据增强；公司同时明确未公布订单排期，客户协议达到披露要求才会公告，因此市场关于长协和订单排期的说法仍不能当作确认事实。

## 2026-05-24 worker batch_no=1 复核

- 公告窗口：北京时间 20:00 后检查公告日期 `2026-05-24` 和 `2026-05-25`，CNINFO 与上交所公司公告查询均返回 0；公司英文 IR 公告页最新可见公告仍为 2026-05-06 的 2026 年 4 月证券变动月报，未见本窗口新增公告。
- 交易事件：按最近 A 股交易日 `2026-05-22` 查询，东方财富龙虎榜返回空；最新历史龙虎榜仍为 2026-03-26。大宗交易按 `2026-05-22` 查询返回空；最新历史大宗交易仍为 2025-09-30。
- Grok/X：Chrome 插件可连接并已为本公司单独打开 Grok 标签页，但 grok.com 触发 Cloudflare 拦截，未取得可用 Grok 结果；未用 Browser Use 或 Playwright 替代登录态 Grok。
- open-web fallback：已按公司名称、ticker、YOFC、optical fiber、datacenter fiber 等词做开放网页检索，仅发现行情页、二级研究/行业讨论及已记录的业绩说明会预告/互动要点延续；未发现新的官方披露、交易事件或可升级为正式事实的观察。
- 状态判断：本轮无新增硬事件写入 `events.jsonl`，核心跟踪假设不变；source gap 更新为 Grok 页面可连接到 Chrome 但被 Cloudflare 拦截，后续仍需在可用登录态 Grok/X 环境补查海外实时讨论与社交热度。

## 2026-05-25 worker batch_no=1 复核

- 公告窗口：北京时间 20:39 后检查公告日期 `2026-05-25` 和 `2026-05-26`，CNINFO `hisAnnouncement/query` 按 `601869,gssh0601869` 及 `长飞光纤` 查询均返回 0；上交所 `queryCompanyBulletinNew.do` 按 `601869` 同窗口返回 0。HKEX `06869.HK` 标题检索最新可见记录仍在本窗口之前，未见 2026-05-25/26 新 H 股公告。
- 交易事件：腾讯行情确认最近 A 股交易日为 `2026-05-25`，收盘价 386.03 元、涨跌幅 -0.68%。东方财富龙虎榜 `RPT_DAILYBILLBOARD_DETAILSNEW` 按 `601869`、`2026-05-25` 查询返回空；大宗交易 `RPT_DATA_BLOCKTRADE` 与 `RPT_BLOCKTRADE_STA` 同日查询均返回空，未发现本轮新增龙虎榜或大宗交易。
- Grok/X：本会话未暴露可调用的 Chrome/Grok 工具命名空间，无法打开独立 Grok/X 标签页；未使用 Browser Use 或 Playwright 替代登录态 Chrome。记录 `grok_status=unavailable_no_callable_chrome_tool_exposed`、`browser_scope=not_available`。
- open-web fallback：已按 `长飞光纤`、`601869`、`YOFC`、`optical fiber`、`datacenter fiber`、`龙虎榜`、`大宗交易` 等词做开放网页检索；仅发现官网/行情页、已记录 IR 信息延续、行业光纤价格/数据中心需求讨论及二级研究观点，未发现新的官方披露、交易硬事件或可升级为确认事实的观察。
- 状态判断：本轮无新增硬事件写入 `events.jsonl`，核心跟踪假设不变；主要 source gap 为当前 worker 无法使用登录态 Chrome/Grok，需要后续在可用 Chrome/Grok 环境补查海外实时讨论、X 线索和数据中心光纤需求的社交热度。

## 2026-05-28 worker batch_no=1 复核

- worker_scope：仅复核 `601869.SH`；未改动 watchlist、日报、`run_status.md` 或 git；`events.jsonl` 未追加。
- run_time：北京时间 `2026-05-28 13:40`，20:00 前；`announcement_window_checked=pending_evening_rescan`，仍需晚间/T+1 补扫 `2026-05-28` 与 `2026-05-29` 公告日期。
- 官方公告：CNINFO `hisAnnouncement/query` 按 `601869,gssh0601869`、公告日期 `2026-05-26~2026-05-28` 返回 0；上交所 `queryCompanyBulletinNew.do` 按 `601869`、`长飞光纤`、`YOFC` 同窗口返回 0；HKEX `06869` 中英文标题检索同窗口返回 0；公司英文 IR 首页最新可见仍为 2026-05-06 月报、2026-04-29 一季报/ESG 相关材料，未见本窗口新增。
- 交易事件：上交所每日交易公开信息 `queryAllTradeOpenDate` 与 `showTradePublicFile` 按已完成交易日 `2026-05-26`、`2026-05-27` 过滤 `601869/长飞` 均无命中；东方财富龙虎榜与大宗交易明细/统计同窗口返回空。`2026-05-28` 仍在盘中，未纳入最终龙虎榜/大宗交易判断。
- Grok/X：本会话无可调用 Chrome/Grok 工具，记录 `grok_status=unavailable_no_callable_chrome_tool_exposed`、`browser_scope=not_available`；已执行普通 open-web fallback，结果仅作 `observation_only`。
- open-web fallback：按 `长飞光纤`、`601869`、`YOFC`、`optical fiber`、`datacenter fiber`、`光纤价格`、`海外扩张`、`特种光纤` 检索，未发现可升级为 601869 正式事实的新公告、订单、价格或客户披露。观察项：长江日报 2026-05-28 报道长进光子 2026-05-27 登陆科创板，并称长飞资本参与战略配售（https://cjrb.cjn.cn/html/2026-05/28/content_151731_3478000.htm）；该项只说明特种光纤生态投资线索，不等同于长飞光纤新增经营披露或业绩贡献确认。
- 状态判断：核心假设不变；光纤价格、数据中心需求、海外扩张和空芯/多芯/特种光纤仍需以后续公司公告、IR 问答、运营商集采价格与财报毛利率验证为准。
- miss_risk：20:00 前运行存在晚间公告漏扫风险；Chrome/Grok 不可用导致海外实时讨论和 X 线索缺口；open-web 中关于涨价、数据中心需求和特种光纤生态的二级材料只能进入观察池，不能替代 CNINFO/SSE/HKEX/公司 IR。

## 2026-05-30 日更状态
- 检查窗口：公告 T/T+1（2026-05-30 至 2026-05-31）、最近交易日龙虎榜/大宗交易（2026-05-29）。
- 结论：补充CNINFO官方PDF作为5月22日投资者关系记录的来源升级；该项仅提升证据等级，不代表新增订单或业绩变化。
- 来源分层：官方披露/交易数据优先；Grok/X不可调用时使用 open-web fallback，相关线索仅作观察池。

## 2026-06-04 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-04` 与 `2026-06-05` 的 T/T+1 检查；CNINFO 名称检索和上交所公告确认新增异常波动公告、年度股东会通知和会议资料，股东会文件偏例行治理事项。
- 新增硬事件：公司股票 2026-06-02 至 2026-06-04 连续三个交易日涨幅偏离值累计超过 20%。公司称日常经营正常、不存在应披露而未披露重大信息；同时承认市场关注光纤光缆价格波动及数据中心需求，但提示价格波动和业绩影响仍不确定。
- 龙虎榜：2026-06-04 因连续三个交易日涨幅偏离值累计达 22.69% 上榜，异常期间成交金额约 259.87 亿元，东方财富席位明细显示净买入约 6.56 亿元。
- 大宗交易：未发现 2026-06-04 新增大宗交易。
- 状态变化：交易风险和预期拥挤度上升；基本面主线不变，光纤涨价、数据中心需求、空芯/多芯光纤仍需要客户、订单、价格和毛利率硬证据。

## 2026-06-23 日更

- 公告窗口：北京时间 20:00 后完成 `2026-06-23` 与 `2026-06-24` 的 T/T+1 检查；CNINFO 公司名/代码查询确认 `2026-06-24` 新增《股票交易异常波动的公告》。
- 新增官方风险边界：公司确认 `2026-06-22` 至 `2026-06-23` 连续两个交易日收盘价格涨幅偏离值累计超过 20%；经自查并向第一大股东中国华信邮电科技有限公司询证，不存在应披露而未披露的重大信息，日常经营正常、行业环境未发生重大变化。公告同时提示 AI 数据中心相关光纤光缆价格波动受关注，但部分产品价格波动及其对公司业绩的影响需结合市场环境和业务结构综合判断。
- 龙虎榜：上交所公开信息参考页与东方财富龙虎榜交叉显示，`2026-06-23` 因连续三个交易日收盘价格涨幅偏离值累计达到 20% 上榜；东方财富明细显示收盘 570.58 元、涨幅 9.9998%、换手率 5.6741%，榜单买入约 23.46 亿元、卖出约 12.53 亿元、净买入约 10.93 亿元。
- 大宗交易：2026-06-23 未发现新增大宗交易。
- open-web：普通联网搜索主要返回龙虎榜、行情、市值和行业景气镜像；未发现比公司公告/交易公开信息更高等级的新增客户、订单、价格或收入拆分证据。
- 状态变化：交易弹性和拥挤度继续上升，同时官方公告对“AI 数据中心光纤光缆价格上涨直接兑现业绩”的叙事作出边界提醒。基本面仍需等待产品价格、业务结构、订单、收入拆分和毛利率在财报或IR中兑现。

## 2026-06-25 最小核验卡：G.657.A2 光棒涨价与 Corning 长单

- 线索来源：Gmail `2026-06-25 08:49`《AI产业链战报》把“G.657.A2 光棒涨价、Corning 锁定 Amazon/NVIDIA/Meta 长单、中国厂商定价权”作为长飞光纤强化信号；该邮件本身为 `mixed-source / lead_only`，只作检索入口，不作公司事实。
- `confirmed_official`：Amazon 官方确认与 Corning 达成多年、数十亿美元协议，由 Corning 供应美国数据中心所需 optical fiber、cable、connectivity solutions；NVIDIA/Corning 官方确认多年商业和技术合作，Corning 将美国 optical connectivity 产能提升 10x、美国光纤产能提升 50% 以上；Meta 官方确认与 Corning 达成最高 60 亿美元多年协议，供应数据中心光纤电缆。这些共同强化“北美云厂商锁定 Corning 光连接/光纤产能”的行业背景。
- `confirmed_official`：长飞 2026Q1 归母净利润 4.95 亿元、同比 +226.40%，扣非归母净利润 4.61 亿元、同比 +966.44%，公司解释为营业收入和毛利率同比增加；2025 年报确认光传输产品收入 83.46 亿元、毛利率 35.90%，光互联组件收入 31.44 亿元、毛利率 39.73%，海外收入 60.92 亿元、占收入 42.74%，海外地区毛利率 35.29%。
- `credible_secondary`：上海证券报/证券时报 2026-06-23 报道称，G.657.A2 类光棒每等效芯公里报价由 22-30 元涨到 160 元、涨幅近 550%，并称紧缺或延续至 2027 年；这属于行业价格与景气证据，不等同于长飞已披露的实现 ASP、订单价格或分产品毛利。
- `lead_only / 未闭合`：邮件中的“中国厂商定价权提升”“长飞光棒产能 5600 吨、自给率 90%+”仍需公司公告、年报产销口径、运营商集采、客户合同或可信数据源补证；本轮在 2025 年报/2026Q1/异动公告中未闭合到长飞披露的 G.657.A2 产销量、订单、客户或单品价格。
- 不能升级为硬事实：Corning 的 Amazon/NVIDIA/Meta 长单是 Corning 自身客户和美国本地产能锁定，不能写成长飞新增 Amazon/NVIDIA/Meta 订单、客户导入或海外收入拆分；G.657.A2 涨价是行业景气和供需背景，不能直接写成长飞新增收入、毛利率兑现或数据中心光纤业务占比提升。
- 状态变化：外部行业信号从“普通光纤价格修复 + 数据中心光连接需求”进一步强化为“高密度数据中心光纤/光连接锁产能观察层增强”；长飞公司层面仍维持 `watch / evidence_gap_card`，核心验证项不变：Q2/Q3 财报毛利率、光传输产品 ASP/销量、G.657.A2 或数据中心光纤订单、海外业务质量、以及是否披露达到公告标准的客户协议。

## 2026-06-28 窄口径补证：光纤上游涨价/供给收紧拆层

- 本轮补证口径：只回答“光纤供不应求再往上游追溯，哪些环节真的可能涨价或收紧”，不把行业景气直接写成长飞经营兑现。
- `credible_secondary`：央视财经报道光纤光缆订单饱满、排产到 2027 年，并把扩产瓶颈指向光纤预制棒；这能抬升“光纤/光缆需求强 + 光棒瓶颈”的行业链条，但未拆到长飞订单或毛利。
- `credible_secondary`：上海证券报/证券时报 2026-06-23 称通用 G.652D 光棒报价涨幅超过 180%，用于数据中心高密度布线的 G.657.A2 类光棒报价由 22-30 元/等效芯公里涨至 160 元/等效芯公里，紧缺或延续至 2027 年。该证据把最强涨价对象从“泛光纤”收敛到“预制棒/光棒，尤其 A2/G.657.A2”。
- `credible_secondary`：上海证券报 2026-06-05 将 A2 类预制棒涨价进一步连接到高纯石英砂、光纤级石英套管、炉芯管、石英管等材料/制品，并给出光纤用高纯砂报价与 6N 以上供给紧张线索；这只能说明上游成本和材料供给压力，不等于长飞采购成本或毛利已受益。
- `confirmed_official`：Amazon、NVIDIA、Meta 与 Corning 的官方长协继续确认北美云厂商在 optical fiber、cable、connectivity solutions / optical connectivity 上锁定供给和扩产；这是高密度数据中心光连接的全球需求与产能锁定证据，不是长飞客户、订单或收入证据。
- `confirmed_official / 公司边界`：长飞 2026-06-24 异动公告确认全球光纤光缆行业处于上行周期，但公司日常经营正常、行业环境未发生重大变化，部分产品价格波动幅度及其对公司业绩影响仍需结合市场环境和业务结构判断。
- 拆层结论：普通光纤/光缆只是需求景气层；光纤预制棒/光棒是当前最明确的涨价和供给收紧层，其中 A2/G.657.A2 强于普通 G.652D；高纯石英砂/石英制品是上游成本压力层；高密度数据中心 optical connectivity/被动件是全球云厂锁产能观察层。四层都不能直接写成长飞订单、收入或毛利兑现。

## 2026-06-30 窄口径补证：中国移动 G.654 高铁用光缆集采价

- 本轮补证口径：只回答“中国移动本次 G.654 高铁用光缆候选价格相对上一次可比集采如何”，不把运营商集采价格直接写成长飞公司经营兑现。
- `confirmed_official`：中国移动采购与招标网 2026-06-26 候选人公示确认，2026-2027 年 G.654 光缆产品（高铁用）集采于 2026-06-25 开标；长飞光纤光缆股份有限公司为第一中标候选人，不含税报价 200,751,151.91 元，拟中标份额 50%；亨通、烽火为第二、第三候选人，拟中标份额分别为 27%、23%。
- `credible_secondary / 折算价`：中国线缆网按本轮采购规模约 64.94 万芯公里和三家候选人拟中标份额加权报价测算，折合光缆均价约 310.74 元/芯公里；其对比 2025 年 G.654.E 光纤光缆集采可比折算价约 188.66 元/芯公里，涨幅约 64.7%。
- 口径边界：本次是中国移动 G.654 光缆产品（高铁用）集采；对比基准是 2025 年 G.654.E 光纤光缆集采折算价。二者都属于运营商高端/特种光缆采购线索，但名称、技术范围和交付结构不完全等同，不能写成普通 G.652D 光纤/光缆、数据中心 G.657.A2 光棒或长飞全产品 ASP 的连续价格指数。
- 公司级未闭合：候选人公示和拟中标份额不等于长飞已确认收入；即使后续正式中标，也仍需合同/订单金额、履约节奏、收入确认、分产品 ASP 和毛利率传导披露，才能把行业/招标价格事实升级到长飞订单、收入或盈利弹性。
- 状态变化：该证据最多把“运营商高端 G.654/G.654.E 光缆集采价格上行”从观察池推进到较强行业价格证据；长飞公司层面仍维持 `watch / evidence_gap_card`，等待公司级订单与毛利率闭合。

## 2026-07-31 日更

- 拟收购长飞上海剩余25%股权、增加6.5亿元关联交易额度，均待交割或实际发生额验证；三日龙虎榜只更新交易风险。
- announcement_window_checked=T_and_T_plus_1；open_web_search_status=searched_no_new_operating_signal。
- thesis_effect=unchanged。

## 2026-08-08 日更

- 公告窗口：回扫公告日期 `2026-08-04` 至 `2026-08-08`；本轮在北京时间 20:00 前运行，`announcement_window_checked=pending_evening_rescan`。
- 资本配置：全资子公司长飞资本拟以自有资金认缴基金 1.70 亿元，占基金认缴规模 34%；基金仍待募集和备案，尚无可确认的投资收益或主营协同。
- 交易：8 月 7 日异动公告确认 8 月 4—6 日连续三日涨幅偏离累计超过 20%；上交所公开信息确认 8 月 5 日振幅、8 月 6 日三日涨幅偏离上榜。
- 相对基线：基金认缴和交易异动均不改变光纤光缆订单、交付、ASP、毛利率和收入确认仍待验证的判断，`thesis_effect=unchanged`。

## 2026-08-14 日更

- 公告窗口：北京时间 20:00 后完成公告日期 `2026-08-14` 与 `2026-08-15` 的 T/T+1 检查。CNINFO 与上交所均确认 T+1 新增《2025年年度A股权益分派实施公告》。
- 资本配置执行：以实施前总股本 827,905,108 股为基数，每股派发现金红利 0.295 元（含税），合计派发 244,232,007 元；其中 A 股以 406,338,314 股为基数、派发 119,869,803 元。A 股股权登记日为 2026-08-20，除权（息）日及现金红利发放日为 2026-08-21。
- 相对基线：这是已获股东会通过的年度分配方案进入执行日程的资本配置事实，不提供数据中心、新型光纤、客户、订单、ASP、毛利率、收入或现金流新增证据；产品商业化阶段不变，`thesis_effect=unchanged`。
- 龙虎榜与大宗交易：2026-08-14 东方财富二级逐家公司查询均未命中；仅作交易数据观察，不能替代上交所一手交易公开信息。
- open-web：独立最近 24 小时检索未见可升级的公司级经营事实；继续跟踪 8 月 21 日实际分派、半年度报告、数据中心相关订单与毛利率。

## 2026-08-21 日更

- 公告窗口：北京时间 20:00 后核验 `2026-08-21` 与 `2026-08-22`；上交所 T+1 披露 2026 年半年度报告及摘要，CNINFO 本窗口未命中；龙虎榜和大宗交易二级查询均为 0。
- 半年报：公司披露营收 98.09 亿元、归母净利润 29.25 亿元、扣非归母净利润 24.49 亿元、经营现金流 18.26 亿元，同比分别增长 53.64%、888.88%、1,680.48% 和 116.82%。报告未经审计。
- 产品与边界：光传输产品收入约 61.26 亿元、光互联组件收入约 22.38 亿元；PMF CPO 相关产品披露正式认证及量产，空芯光纤累计交付超 10,000 芯公里并覆盖 13 个以上商用/试点项目。公司尚未披露各项的客户、订单、毛利和单独收入，不能外推全部利润来源。
- 资本配置：董事会拟每 10 股派发现金红利 10.60 元，尚待股东会审议。相对基线，`YOFC-01`、`YOFC-02`、`YOFC-03` 获正式披露增强，但客户和分产品盈利仍待验证。

## 2026-08-24 日更（T+1）

- 公告窗口：CNINFO 以代码与 `orgId` 主源查询命中 2026-08-25 两份 T+1 文件；公司将于 2026-09-11 召开第二次临时股东会，审议 2026 年中期利润分配方案及 2026 年 H 股股份奖励计划等议案。
- 资本配置：中期利润分配和 H 股奖励计划均仍待股东会批准；奖励股份上限为 1,000 万股、占现有已发行股份约 1.21%，拟由受托人从二级市场购买既有 H 股作为奖励来源。该事项尚不代表实际分红、授予、发行新股或主营经营兑现。
- 相对基线：新增治理/资本配置时间表，不改变光传输、PMF CPO、空芯光纤的客户、订单、收入、毛利率或商业化阶段判断，`thesis_effect=unchanged`。

## 2026-08-25 日更

- 公告窗口：北京时间 20:00 后完成 `2026-08-25` 与 `2026-08-26` 的 T/T+1 核验；CNINFO 和上交所公司公告本窗口均无新增，公司于 8 月 25 日发布的股东会通知已在上一轮 T+1 台账中存在，不重复写入。
- 龙虎榜：上交所交易公开信息确认公司因日价格振幅达到 15% 上榜，异常期间振幅 19.16%，成交 20,903,179 股、成交额 80.67 亿元；东方财富二级明细交叉显示榜单净买入约 1.41 亿元、4 家机构买入。大宗交易查询无新增。
- 相对基线：这是交易波动与资金结构证据，不构成光传输、光互联组件、PMF CPO、空芯光纤的客户、订单、收入、毛利率或商业化阶段新增证据；基本面 `thesis_effect=unchanged`，继续等待分产品收入、客户订单和现金流验证。

## 2026-09-03 晚间日更

- 公告窗口：CNINFO 与 SSE 公司公告接口对 2026-09-03 至 2026-09-04 均未命中发行人公告。
- 龙虎榜/大宗交易：SSE 当日交易公开信息未命中；沪市逐证券大宗交易官方回读仍为 source_gap，二级空结果只作观察。
- 相对基线：未出现光纤需求、海外项目、光传输或新型光纤客户订单与收入的新硬证据，thesis_effect=unchanged。

## 2026-09-05 日更（20:00 前）

- CNINFO 与 SSE 公司公告接口对 2026-09-04 至 2026-09-05 均未命中；SSE 交易公开信息未命中。
- 沪市逐证券大宗交易官方回读为 source_gap，二级 `code=9201` 空结果不等于官方零记录；未发现可入账的经营或交易事实。
- 相对基线：光纤需求、海外项目、新型光纤客户订单与收入仍缺新增硬证据，`thesis_effect=unchanged`；晚间复扫待完成。
