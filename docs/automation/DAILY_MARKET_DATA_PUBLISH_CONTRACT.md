...
4. 两仓开始时用 `scripts/automation_worktree_guard.py snapshot` 将完整 Git 状态写到任务目录外的快照文件。开始前已存在的非发布路径改动与未跟踪文件一律保留、不纳入暂存，也不阻止本轮。将要发布的白名单产物即使开始前已有改动，也不因此阻断对应链路；必须保留起始快照，对每个白名单路径逐一记录运行前版本与本轮后版本差异。提交时仅暂存已验证且属于本轮更新的白名单路径，不得把无法区分或不属于本轮的既有改动混入提交；若无法可靠区分，则停止该路径的提交并说明原因。唯一受控输入例外是 `watchlists/AI产业链.xlsx`：仅当本轮确认其成分集合变化时，才可作为本轮白名单的一部分。
