...
The child automation configuration lives outside the prompt body. Every child created or updated from this template must use `model="gpt-5.6-terra"` and `reasoning_effort="ultra"` (Codex UI: `5.6 terra` / `ULTRA`). Do not copy these configuration fields into the generated child prompt header.
