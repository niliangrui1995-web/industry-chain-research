...
10. 每个 child 必须是独立 cron，name=财报电话会深挖 {ticker} {company} {report_date}，cwd=D:\vcp_hunter\产业链投研，executionEnvironment=local，model=gpt-5.5，reasoningEffort=xhigh（TOML 字段名为 reasoning_effort = "xhigh"）。prompt 必须按 CHILD_PROMPT_TEMPLATE.md 填充；child prompt 正文使用英文，仅最终输出要求中文，并包含 literal CHILD TASK SKILL HARD GATE、Calendar source/Event status/Source confidence/Official source URL/Calendar caveat、baseline、downstream demand outlook、upstream bottleneck 和 source-quality 规则。
