...
| `allstock-data` + `tdx-finance-data` + `banana-farmer` | Quotes, K-lines, liquidity, valuation, technical indicators, limit-board/theme heat, trend, timing, and risk context | Cannot prove beneficiary status; TDX limit-board reasons and concept labels are secondary trading context, not official evidence |
...
|---|---|---|
| News/rumor/order/product launch/price hike/supply-chain change -> small-cap alpha or financial-statement transmission | `industry-research-router` + `search-specialist` / `ai-chain-research-orchestrator` + `serenity-alpha` + `allstock-data` / `tdx-finance-data` + `advanced-evaluation` | First prove observable demand and source quality, then translate to financial lines and validation conditions. Use TDX only for A-share market reaction, valuation, technical, theme, or limit-board context. |
...
| 最近24/48/72小时 AI 产业链消息、爆料、涨价、短缺、停产 | `industry-research-router` + `ai-chain-research-orchestrator` + `browser-grok-gemini-research` + `semiconductor-ai-chain-investment-researcher` + `allstock-data` / `tdx-finance-data` | Grok/X first for X-native discovery. Gemini is used when requested or useful for source gaps/counter-evidence/Gemini web Deep Research. Codex verifies and concludes. Use TDX only after source discovery to check A-share market reaction. |
...
| 单只股票能买吗、估值、买卖点、仓位 | `industry-research-router` + `stock-evaluator` + `tdx-finance-data` / market data skill | Never fabricate numbers. Do not substitute HTSC quick diagnosis or TDX market context for the upstream/downstream, demand pass-through, core-product price/unit-economics, and official-evidence checks. |
...
| 谁股票弹性最大、短线弹性、交易弹性 | `industry-research-router` + `allstock-data` + `tdx-finance-data` + `banana-farmer` + `advanced-evaluation` | Prioritize float market cap, turnover, volatility, 20/60-day trend, catalyst density, crowding risk. Use TDX for A-share valuation, turnover, technical indicators, sector/theme heat, and limit-board status. |
| A股公司持续跟踪、watchlist日更、baseline/state/events维护 | `industry-research-router` + `a-share-company-tracking` + `a-share-disclosure-trading-data` + `search-specialist` + `research-summarizer`; add `tdx-finance-data` when market reaction matters | At or after 20:00 Beijing time, check both announcement date T and T+1. Keep one company per worker when sub-agent tools are available. TDX fields stay in market-reaction notes, not official-evidence ledgers. |
| A股公告、CNINFO、交易所披露、龙虎榜、大宗交易核验 | `industry-research-router` + `a-share-disclosure-trading-data` + `search-specialist`; add `tdx-finance-data` / `allstock-data` for market reaction | Treat trading events as liquidity/elasticity signals, not fundamental proof. TDX can supplement涨跌停、封单、连板、板型、涨停原因/原因揭秘, but those are secondary trading context. |
| A股涨停/跌停、封单、连板、板型、涨停原因、概念热度、ETF/基金快照 | `tdx-finance-data` + `industry-research-router`; add `a-share-disclosure-trading-data` / `search-specialist` if claims need official proof | Use `mcp__tdx.tdx_wenda_quotes` style natural-language queries. Label quote/valuation/technical/ETF data as `market_data_vendor`; label reasons/concepts/limit-board narratives as `secondary_trading_context`. |
| A股/港股/美股实时行情、K线、盘口 | `tdx-finance-data` for A-share/HK/fund/index/sector natural-language snapshots; `allstock-data` for CN/HK/US quick checks and order-book style fields; `yfinance-mcp-server` / `stocks` for global; `alpha-vantage` for indicators | Tencent API returns GBK text and may delay up to 15 minutes. TDX is not an order-book or historical warehouse. Always normalize exchange/ticker suffix. |
| A股最新价、PE/PB、成交额、换手率、多股指标对比 | `tdx-finance-data` / `query-indicator`; fall back to `allstock-data` / other market-data skills if unavailable | Fast indicator lookup only. Prefer single-security or one-screening-question TDX queries because multi-stock comparison can omit names. Record date/time and do not use market data to prove beneficiary status. |
...
- TDX Finance Data MCP outputs are supplemental market-data-vendor or secondary-trading-context evidence. Use it for A-share/HK/fund/index/sector natural-language quote tables, valuation, turnover, technical indicators, ETF/fund snapshots, sector/theme constituents, and limit-board context. Do not use it as official disclosure, research-report search, capital-flow truth source, order-book source, bulk multi-stock comparator, historical OHLCV warehouse, factor/backtest source, or beneficiary proof.
- Prefer narrow TDX queries: one security, one sector/index, one ETF/fund, or one clearly scoped screening question. If a multi-security answer omits a target, split the request and label missing rows instead of inferring.
...
| `tdx-finance-data` | TDX MCP natural-language finance data via `mcp__tdx.tdx_wenda_quotes`: A-share/HK/fund/index/sector quote tables, valuation, turnover, technical indicators, ETF/fund snapshots, sector/theme constituents,涨跌停、封单、连板、板型、涨停原因/原因揭秘. Suitable for market reaction and trading-context checks; unsuitable as official disclosure, report retrieval, reliable资金流 source, order book, bulk multi-stock comparator, or backtest data source. |
...
|---|---|
| "Which company has the most alpha from this news?" | `industry-research-router` + `search-specialist` / `ai-chain-research-orchestrator` + `serenity-alpha` + `allstock-data` / `tdx-finance-data` + `advanced-evaluation` |
...
| “AI产业链最近48小时消息/爆料并映射股票？” | `industry-research-router` + `ai-chain-research-orchestrator` + `browser-grok-gemini-research` + `semiconductor-ai-chain-investment-researcher` + `allstock-data` / `tdx-finance-data` |
...
| “谁股票弹性最大？” | `industry-research-router` + `allstock-data` + `tdx-finance-data` + `banana-farmer` + `advanced-evaluation` |
| “跑一下A股公司跟踪/更新watchlist公司状态” | `industry-research-router` + `a-share-company-tracking` + `a-share-disclosure-trading-data`; add `tdx-finance-data` when market reaction matters |
| “查这几家公司今晚有没有公告/龙虎榜/大宗交易” | `industry-research-router` + `a-share-disclosure-trading-data` + `search-specialist`; add `tdx-finance-data` for market reaction and limit-board context |
| “查A股涨停/跌停、封单、连板、板型、涨停原因/概念热度” | `industry-research-router` + `tdx-finance-data`; add `a-share-disclosure-trading-data` / `search-specialist` for official-proof follow-up |
...
| “红利低波现在有没有股债性价比？” | `industry-research-router` + `dividend-premium-tracker` + `stock-evaluator` + `tdx-finance-data` / market data skill |
