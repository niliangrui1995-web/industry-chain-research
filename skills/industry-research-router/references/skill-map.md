...
| 成长行业瓶颈、BOM、交货时滞、HHI、定价权、利润池迁移、纯度筛股、用户提供 CSV/JSON/XLSX 研究数据 | `industry-research-router` + `industry-chain-deep-disassembly`; load its matching adapter for PCB/CCL, 液冷, 光模块, or 数据中心电力; use its data-interface normalizer for raw files; add `search-specialist` when hard-source discovery is needed; add `stock-evaluator` / market-data skills after node selection | Start from terminal demand and chain nodes, normalize evidence tables first, and avoid starting from hot stock names. |
...
| `industry-chain-deep-disassembly` | Cross-industry deep disassembly for demand transmission, BOM/value nodes, lead-time bottlenecks, HHI concentration, profit-pool migration, pure-play filtering, financial confirmation, user-provided data normalization, and adapter-guided PCB/CCL, liquid-cooling, optical-module, or data-center-power node mapping. |
