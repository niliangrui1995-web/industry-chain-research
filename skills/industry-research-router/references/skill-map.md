...
| AI产业链最新消息、Grok/X爆料、A股映射、必要时Gemini反证 | `industry-research-router` + `ai-chain-research-orchestrator` + `finance-news` + `allstock-data` + `advanced-evaluation` | Default to Grok/X discovery, Codex web verification, and local market data; use Gemini only for major unresolved or conflicting lines. |
...
| `ai-chain-research-orchestrator` | AI supply-chain latest-news workflow that coordinates Grok/X discovery, Codex web checks, local market data, ranked stock mapping, and optional Gemini counter-checks. |
...
| “用Grok/X帮我抓AI产业链最近48小时消息并映射股票，必要时再用Gemini反证” | `industry-research-router` + `ai-chain-research-orchestrator` + `finance-news` + `allstock-data` |
