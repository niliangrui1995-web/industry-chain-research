...
- AI supply-chain latest-news and rumor verification: `ai-chain-research-orchestrator` with parallel mandatory Grok/X native X-only collection and Perplexity core open-web news/source collection, then Codex supplemental web verification, plus `finance-news`, `web-scraper`, `allstock-data`, and `advanced-evaluation` as needed.
...
2. Use current data when facts may have changed:
   ...
   - Treat X/Twitter, Grok, Perplexity, Gemini, and similar assistant/search products as evidence collection only; Codex must do the final synthesis, evidence weighting, risk judgment, stock mapping, and conclusion.
