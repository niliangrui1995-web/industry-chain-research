...
description: "Structured source summarization for industry research, company research, annual and quarterly reports, whitepapers, PDFs, web articles, and multi-source evidence briefs. Extracts key findings, claim-evidence links, limitations, contradictions, and citations. Use when the user asks to summarize or compare sources, digest filings or reports, build a research brief, or turn collected evidence into a structured note before final investment synthesis."
---
...
- Read `references/summary-templates.md` only when a strict source-summary template is useful. Use `scripts/extract_citations.py` only for local text files where citation extraction is explicitly needed.
